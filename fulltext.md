...
|   |    EnableTermsSplit   |   bool   | Enable splitting query terms. e.g. term "dicaprio" will match words "di" and "caprio". Terms with '-' and '+' prefixes will not be splitted                                                                                                                                                                                                                                 |     true      |
...
|   |     Synonyms          | []struct | A list of synonyms to be used in full text search            
                                                                              |               |
|   |     TermsBoost        | []struct | A list of terms boosts to be used in full text search. The relevance of each term will be increased proportionally to the specified boost factor. For each term, all possible word forms are boosted, generated using all the stemmers listed in 'Stemmers'          
                                                                              |               |
...
|   | SplitProc                    |    int   | Base relevancy of splitted terms match                            |      90       |
