...
Optionaly `radius` could be specified for filtering vectors by metric.
...
Parameters set for `KNN`-query depends on the specific index type. It is required to specify `k` or `radius` (or both) for every idex type. `k` is the maximum number of documents returned from the index for subsequent filtering.
