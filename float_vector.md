...
The initial size `start_size` can optionally be specified for `brute force` и `hnsw` indexes, which helps to avoid reallocations and reindexing.
...
## KNN search
The only filtering operation of float vector fields is `KNN`.
