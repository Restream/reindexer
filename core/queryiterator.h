...
class QueryIterator : public SelectKeyResult {
public:
	enum {
		Forward,
		Reverse,
	...
	QueryIterator(const SelectKeyResult &res, OpType _op, bool _distinct, const string &_name, bool forcedFirst = false)
	...
	bool TryCompare(const PayloadData *item, int idx) {
