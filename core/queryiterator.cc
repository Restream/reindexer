...
void QueryIterator::Bind(const PayloadType *type, int field) {
	for (auto &cmp : comparators_) cmp.Bind(type, field);
}

void QueryIterator::Start(bool reverse) {
	reverse_ = reverse;
	lastIt_ = begin();
...
// ***************************
bool QueryIterator::nextFwd(IdType minHint) {
...
bool QueryIterator::nextRev(IdType maxHint) {
...
// ********************************
bool QueryIterator::nextFwdSingleIdset(IdType minHint) {
	if (minHint > lastVal_) lastVal_ = minHint - 1;
...
bool QueryIterator::nextRevSingleIdset(IdType maxHint) {
	if (maxHint < lastVal_) lastVal_ = maxHint + 1;
...
// ********************************
bool QueryIterator::nextFwdSingleRange(IdType minHint) {
	if (minHint > lastVal_) lastVal_ = minHint - 1;
...
bool QueryIterator::nextRevSingleRange(IdType maxHint) {
	if (maxHint < lastVal_) lastVal_ = maxHint + 1;
...
// ********************************
bool QueryIterator::nextUnsorted() {
	if (lastIt_ == end()) {
		return false;
...
void QueryIterator::ExcludeLastSet() {
	if (!End() && lastIt_ != end()) {
...
void QueryIterator::Append(SelectKeyResult &other) {
	for (auto &r : other) push_back(std::move(r));
	for (auto &c : other.comparators_) comparators_.push_back(std::move(c));
}

double QueryIterator::Cost(int expectedIterations) const {
...
void QueryIterator::SetExpectMaxIterations(int expectedIterations) {
	for (auto &r : *this) {
...
int QueryIterator::GetMaxIterations() const {
	int cnt = 0;
