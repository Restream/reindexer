...
KeyRef IndexOrdered<T>::Upsert(const KeyRef &key, IdType id) {
	...
	if (keyIt == this->idx_map.end() || !found)
		keyIt = this->idx_map.insert(keyIt, {(typename T::key_type)(KeyValue)key, Index::KeyEntry()});
...
template <>
str_map<Index::KeyEntry>::iterator IndexOrdered<str_map<Index::KeyEntry>>::lower_bound(const KeyRef &key, bool &found) {
	p_string skey = (p_string)key;
	tmpKeyVal_->assign(skey.data(), skey.length());
	auto it = this->idx_map.lower_bound(tmpKeyVal_);
	found = (it != this->idx_map.end() && tmpKeyVal_ == it->first);
	return it;
}

template <typename T>
typename T::iterator IndexOrdered<T>::lower_bound(const KeyRef &key, bool &found) {
	auto it = this->idx_map.lower_bound((typename T::key_type)key);
	found = (it != this->idx_map.end() && !this->idx_map.key_comp()((typename T::key_type)key, it->first));
	return it;
}
...
SelectKeyResults IndexOrdered<T>::SelectKey(const KeyValues &keys, CondType condition, SortType sortId, Index::ResultType res_type) {
	...
	switch (condition) {
		case CondLt:
			endIt = this->idx_map.lower_bound((typename T::key_type)key1);
			break;
		case CondLe:
			endIt = this->idx_map.lower_bound((typename T::key_type)key1);
			if (endIt != this->idx_map.end() && !this->idx_map.key_comp()((typename T::key_type)key1, endIt->first)) endIt++;
			break;
		case CondGt:
			startIt = this->idx_map.upper_bound((typename T::key_type)key1);
			break;
		case CondGe:
			startIt = this->idx_map.find((typename T::key_type)key1);
			if (startIt == this->idx_map.end()) startIt = this->idx_map.upper_bound((typename T::key_type)key1);
			break;
		case CondRange: {
			if (keys.size() != 2) throw Error(errParams, "For ranged query reuqired 2 arguments, but provided %d", (int)keys.size());
			auto key2 = keys[1];

			if (this->idx_map.key_comp()((typename T::key_type)key2, (typename T::key_type)key1)) {
			...
			endIt = this->idx_map.lower_bound((typename T::key_type)key2);
			if (endIt != this->idx_map.end() && !this->idx_map.key_comp()((typename T::key_type)key2, endIt->first)) endIt++;
		} break;
		default:
		...
		for (auto id : keyIt.second.Unsorted()) {
			if (id > (int)ids2Sorts.size() || ids2Sorts[id] == SortIdUnexists) {
				logPrintf(
					...
					this->name.c_str(), KeyValue(keyIt.first).toString().c_str(), (int)id, (int)totalIds);
				this->DumpKeys();
				assert(0);
			}
...
template class IndexOrdered<map<PayloadData, Index::KeyEntry, less_composite>>;
template class IndexOrdered<map<double, Index::KeyEntry>>;
template class IndexOrdered<str_map<Index::KeyEntry>>;
template class IndexOrdered<map<int, Index::KeyEntry>>;
template class IndexOrdered<map<int64_t, Index::KeyEntry>>;
