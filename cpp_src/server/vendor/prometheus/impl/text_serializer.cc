...
const std::string& EscapeLabelValue(const std::string& value, std::string* tmp) {
	bool copy = false;
...
void WriteHead(std::ostream& out, const MetricFamily& family, const ClientMetric& metric, const std::string& suffix = "",
			   const std::string& extraLabelName = "", const std::string& extraLabelValue = "") {
	...
	if (!metric.label.empty() || !extraLabelName.empty()) {
		...
		if (!extraLabelName.empty()) {
			out << prefix << extraLabelName << "=\"" << EscapeLabelValue(extraLabelValue, &tmp) << "\"";
		}
		out << "}";
	}
	out << " ";
...
void SerializeSummary(std::ostream& out, const MetricFamily& family, const ClientMetric& metric) {
...
void SerializeUntyped(std::ostream& out, const MetricFamily& family, const ClientMetric& metric) {
	WriteHead(out, family, metric);
