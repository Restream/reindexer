...
Server::Server() : logLevel_(LogNone), storageLoaded_(false), running_(false) {}

int Server::main(int argc, char *argv[]) {
	errno = 0;
	Server svc;
	Error err = svc.InitFromCLI(argc, argv);
	if (!err.ok()) {
...
Error Server::InitFromCLI(int argc, char *argv[]) {
...
Error Server::InitFromFile(const char *filePath) {
...
Error Server::InitFromYAML(const string &yaml) {
