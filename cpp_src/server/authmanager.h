...
public:
	...
	[[nodiscard]] bool Check(const std::string& user, const std::string& passwd) const RX_REQUIRES(!mtx_);
...
private:
	class [[nodiscard]] Token {
		...
		[[nodiscard]] static std::string_view generate(const std::string& passwd, unsigned char* hash);
		[[nodiscard]] bool check(std::string_view token, const AuthManager& host) const noexcept RX_REQUIRES_SHARED(host.mtx_);
	...
	[[nodiscard]] auto findToken(const std::string& user) const&;
	[[nodiscard]] auto findToken(const std::string& user) &;
