...
private:
	...
	int modifyQueryTxImpl(http::Context& ctx, const std::string& dbName, std::string_view txId, impl::Query& q);
	...
	Error execQueryByType(const impl::Query& query, reindexer::QueryResults& res, http::Context& ctx);
