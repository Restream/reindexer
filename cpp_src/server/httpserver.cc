...
Error HTTPServer::execQueryByType(const reindexer::impl::Query& query, reindexer::QueryResults& res, http::Context& ctx) {
...
int HTTPServer::GetSQLQuery(http::Context& ctx) {
	...
	try {
		query = impl::Query::FromSQL(sqlQuery);
...
int HTTPServer::queryConvert(http::Context& ctx, QueryFormat incomingFormat) {
	...
	try {
		const auto q = incomingFormat == QueryFormat::Sql ? impl::Query::FromSQL(strQuery) : impl::Query::FromJSON(strQuery);
...
int HTTPServer::PostQuery(http::Context& ctx) {
	auto q = impl::Query::CreateEmpty();
	try {
		q = impl::Query::FromJSON(ctx.body->Read());
	} catch (Error& err) {
		return status(ctx, http::HttpStatus(err));
	}
...
int HTTPServer::GetItems(http::Context& ctx) {
	...
	impl::Query query = impl::Query::FromSQL(querySer.Slice());
	if (ctx.request->params.Get("format"sv) != kCSVFileFmt) {
		query.CalcTotal(ModeAccurateTotal);
	}
...
int HTTPServer::modifyQueryTxImpl(http::Context& ctx, const std::string& dbName, std::string_view txId, impl::Query& q) {
	reindexer::QueryResults res;
	auto tx = getTx(dbName, txId);
...
int HTTPServer::GetSQLQueryTx(http::Context& ctx) {
	...
	try {
		auto q = impl::Query::FromSQL(sqlQuery);
		switch (q.Type()) {
			case QueryDelete:
			case QueryUpdate:
		...
		return status(ctx, http::HttpStatus(Error(errLogic, "Unexpected query type: {}", int(q.Type()))));
	} catch (const Error& e) {
