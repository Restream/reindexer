...
Error RPCServer::execSqlQueryByType(std::string_view sqlQuery, QueryResults& res, int fetchLimit, cproto::Context& ctx) noexcept {
	try {
		const auto q = impl::Query::FromSQL(sqlQuery);
		switch (q.Type()) {
			case QuerySelect:
...
Error RPCServer::DeleteQueryTx(cproto::Context& ctx, p_string queryBin, int64_t txID) noexcept {
	try {
		...
		auto query = impl::Query::Deserialize(ser);
...
Error RPCServer::Select(cproto::Context& ctx, p_string queryBin, int flags, int limit, p_string tmVersionsPck) {
	...
	try {
		query = impl::Query::Deserialize(ser);
	} catch (Error& err) {
		return err;
	}
	...
	if (query.IsWALQuery()) {
		query.AppendQueryEntry<QueryEntry>(OpAnd, "#slave_version", CondEq, VariantArray::Create(data->rxVersion.StrippedString()));
	}
