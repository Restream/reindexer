...
Index* parse_coarse_quantizer(
    ...
    if (match("IVF([0-9]+[kM]?)\\(Index([0-9])\\)")) {
        ...
        FAISS_ASSERT(no >= 0 && size_t(no) < parenthesis_indexes.size());
...
std::unique_ptr<Index> index_factory_sub(
    ...
    if (verbose) {
        printf("after () normalization: %s %llu parenthesis indexes d=%d\n",
               description.c_str(),
               static_cast<unsigned long long>(parenthesis_indexes.size()),
               d);
    }
