...
struct IndexFlatCodes : Index {
    ...
    /// normalization coefficients (for cosine metric)
    std::vector<float> norm_coefs;

    IndexFlatCodes();

    IndexFlatCodes(size_t code_size, idx_t d, MetricType metric = METRIC_L2, bool is_cosine = false);
