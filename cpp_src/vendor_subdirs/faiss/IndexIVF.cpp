...
size_t Level1Quantizer::allocated_mem_size() const noexcept
{
    // Do not count clustering_index (assuming we do not own it)
    return (own_fields && quantizer) ? quantizer->allocated_mem_size() : 0;
}
...
IndexIVF::IndexIVF(
        ...
        MetricType metric,
        bool is_cosine)
        : Index(d, metric, is_cosine),
          IndexIVFInterface(quantizer, nlist),
          invlists(new ArrayInvertedLists(nlist, code_size, is_cosine)),
          own_invlists(true),
          code_size(code_size) {
...
void IndexIVF::add(idx_t n, const float* x) {
    add_with_ids(n, x, nullptr, nullptr);
}

void IndexIVF::add_with_ids(idx_t n, const float* x, const idx_t* xids) {
    IndexIVF::add_with_ids(n, x, nullptr, xids);
}

void IndexIVF::add_with_ids(idx_t n, const float* x, const float *x_norms, const idx_t* xids) {
    std::unique_ptr<idx_t[]> coarse_idx(new idx_t[n]);
    if (is_cosine) {
        auto x_normalized = std::make_unique<float[]>(n * d);
        if (x_norms) {
            std::memcpy(x_normalized.get(), x, n * d * sizeof(float));
            auto nptr = x_normalized.get();
            for (size_t i = 0; i < n; ++i, nptr += d) {
                const auto norm_coef = x_norms[i];
                for (size_t j = 0; j < d; ++j) {
                    nptr[j] *= norm_coef;
                }
            }
            quantizer->assign(n, x_normalized.get(), coarse_idx.get());
            add_core(n, x, x_norms, xids, coarse_idx.get());
        } else {
            auto x_norm_coefs = std::make_unique<float[]>(n);
            std::memcpy(x_normalized.get(), x, n * d * sizeof(float));
            auto nptr = x_normalized.get();
            for (size_t i = 0; i < n; ++i, nptr += d) {
                x_norm_coefs.get()[i] = reindexer::ann::NormalizeVector(nptr, d);
            }
            quantizer->assign(n, x_normalized.get(), coarse_idx.get());
            add_core(n, x, x_norm_coefs.get(), xids, coarse_idx.get());
        }
    } else {
...
void IndexIVF::add_core(idx_t n,
                        const float* x,
                        const float* x_norms,
                        const idx_t* xids,
...
#pragma omp parallel for if (nt > 1)
        ...
        // all)
        IndexIVFStats local_stats;
        // FIXME: !!! Disabled stats
        // indexIVF_stats are not threadsafe. We do not use the currently
        sub_search_func(n, x, distances, labels, &local_stats);
    }
}
        ...
        auto scan_one_list = [&](idx_t key,
            ...
            try {
                if (invlists->use_iterator) {
                    ...
                    InvertedLists::ScopedNorms snorm_coefs(invlists, key);
                    const float* norm_coefs = snorm_coefs.get();
                    ...
                    nheap += scanner->scan_codes(
                            list_size, codes, norm_coefs, ids, simi, idxi, k);
            ...
            try {
                ...
                if (invlists->use_iterator) {
                    ...
                    scanner->scan_codes_range(
                            list_size, scodes.get(), snorm_coefs.get(), ids.get(), radius, qres);
                }
                nlistv++;
                ndis += list_size;
            } catch (const std::exception& e) {
...
size_t IndexIVF::allocated_mem_size() const {
    size_t ret = IndexIVFInterface::allocated_mem_size() + direct_map.allocated_mem_size();
    if (own_invlists && invlists) {
        ret += invlists->allocated_mem_size();
    }
    return ret;
}
...
void IndexIVF::update_vectors(int n, const idx_t* new_ids, const float* x) {
    if (direct_map.type == DirectMap::Hashtable) {
        ...
        add_with_ids(n, x, nullptr, new_ids);
        return;
    }
...
size_t InvertedListScanner::scan_codes(size_t list_size,
                                       const uint8_t* codes,
                                       const float* norm_coefs,
                                       const idx_t* ids,
                                       float* simi,
                                       idx_t* idxi,
                                       size_t k) const {
