...
IndexFlatCodes::IndexFlatCodes(size_t code_size, idx_t d, MetricType metric, bool is_cosine)
        : Index(d, metric, is_cosine), code_size(code_size) {}
...
void IndexFlatCodes::add(idx_t n, const float* x) {
    ...
    if (is_cosine) {
        ...
        norm_coefs.resize(ntotal + n);
        for (size_t i = 0; i < n; i++) {
            norm_coefs[ntotal + i] = reindexer::ann::NormalizeVector(x_normalized.get() + i * d, d);
        }
    }
...
size_t IndexFlatCodes::allocated_mem_size() const
{
   return codes.capacity() * sizeof(uint8_t) + norm_coefs.capacity() * sizeof(float);
}
