...
struct Level1Quantizer {
    ...
    size_t allocated_mem_size() const noexcept;
...
struct IndexIVF : Index, IndexIVFInterface {
    ...
    void add_with_ids(idx_t n, const float* x, const float* x_norms, const idx_t* xids);
...
struct InvertedListScanner {
     ...
     * @param norm_coefs normaliztion coeffs for codes (for cosine)
...
// FIXME: !!! indexIVF_stats do not work with our threading model. If we are gona use them, we should add some synchroniztion
// Currently those stats are commented in IVFFlatIndex routes
