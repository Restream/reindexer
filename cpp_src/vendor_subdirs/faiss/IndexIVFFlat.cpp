...
IndexIVFFlat::IndexIVFFlat(Index* quantizer,
                           ...
                           MetricType metric,
                           bool is_cosine)
        : IndexIVF(quantizer, d, nlist, sizeof(float) * d, metric, is_cosine) {
...
void IndexIVFFlat::train(idx_t n, const float* x) {
    IndexIVFFlat::train(n, x, nullptr);
}

void IndexIVFFlat::train(idx_t n, const float* x, const float* x_norms) {
    if (is_cosine) {
        std::unique_ptr<float[]> x_normalized;
        if (x_norms) {
            x_normalized = std::make_unique<float[]>(n * d);;
            ...
            x_normalized = reindexer::ann::NormalizeCopyVectors(x, n, d);
        }
        // use normalized data for cosine
...
#pragma omp parallel reduction(+ : n_add)
    {
        ...
        for (size_t i = 0; i < n; i++) {
            ...
            if (list_no >= 0 && list_no % nt == rank) {
                ...
                const float* xi_norm = (x_norms == nullptr) ? nullptr : (x_norms + i);
                size_t offset = invlists->add_entry(
                        list_no, id, (const uint8_t*)xi, inverted_list_context, xi_norm);
...
size_t IndexIVFFlat::allocated_mem_size() const {
    return IndexIVF::allocated_mem_size();
}

size_t IndexIVFFlat::unique_ids_count() const noexcept {
    return direct_map.unique_ids_count();
}
...
reindexer::ConstFloatVectorView IndexIVFFlat::getView(idx_t key) const noexcept {
    const idx_t lo = direct_map.get(key);
    const auto listNo = lo_listno(lo);
    const auto offset = lo_offset(lo);
    return reindexer::ConstFloatVectorView{std::span(reinterpret_cast<const float*>(invlists->get_single_code(listNo, offset)) ,d )};
}
            ...
            if (sel.is_member(idsi[j])) {
                if (replace.count(idsi[j]) == 0) {
                    ...
                    invlists->update_entry(
                            ...
                            InvertedLists::ScopedNorms(invlists, i, l).get());
                } else {
                    invlists->update_entry(
                            i,
                            j,
