...
struct Index {
    ...
    bool is_cosine; ///< Extra flag for cosine metric (implemented via Inner product)
    float metric_arg; ///< argument of the metric type

    explicit Index(idx_t d = 0, MetricType metric = METRIC_L2, bool _is_cosine = false)
            : d(d),
              ntotal(0),
              verbose(false),
              is_trained(true),
              metric_type(metric),
              is_cosine(_is_cosine),
              metric_arg(0) {}
