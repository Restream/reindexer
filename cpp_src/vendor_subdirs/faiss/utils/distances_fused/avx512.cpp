...
void kernel(
    ...
    _mm_prefetch(reinterpret_cast<const char*>(xd_0 + DIM * sizeof(float)), _MM_HINT_NTA);
    ...
    for (size_t nx_k = 0; nx_k < NX_POINTS_PER_LOOP; nx_k++) {
        ...
        _mm512_storeu_si512(
            reinterpret_cast<__m512i*>(min_indices_scalar), min_indices_i[nx_k]);
...
#pragma omp parallel for schedule(dynamic)
    for (int64_t i = 0; i < int64_t(nx_p); i += NX_POINTS_PER_LOOP) {
