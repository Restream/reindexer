...
void fvec_inner_products_ny_ref(
...
#if 0
    {
        ...
        FINTEGER onei = 1;
        sgemv_dlwrp_ ("T", &di, &nyi, &one, y, &di, x, &onei, &zero, ip, &onei);
    }
#endif
    for (size_t i = 0; i < ny; i++) {
...
RX_ALWAYS_INLINE static float fvec_norm_L2sqr_def(const float* x, size_t d) {
...
FAISS_PRAGMA_IMPRECISE_FUNCTION_BEGIN
RX_AVX_TARGET_ATTR
static float fvec_norm_L2sqr_avx(const float* x, size_t d) {
...
static float fvec_norm_L2sqr_avx512(const float* x, size_t d) {
    // Autovectorized version
    float res = 0;
    FAISS_PRAGMA_IMPRECISE_LOOP
    for (size_t i = 0; i != d; ++i) {
...
using NormL2SqrPtrT = float (*)(const float*, size_t);
...
#endif // REINDEXER_WITH_SSE
    return fvec_norm_L2sqr_def;
}

NormL2SqrPtrT fvec_norm_L2sqr_ptr = initNormL2SqrSIMD();
}
...
static inline int fvec_madd_and_argmin_sse(
    ...
    n >>= 2;
    __m128 bf4 = _mm_set_ps1(bf);
    __m128 vmin4 = _mm_set_ps1(1e20f);
