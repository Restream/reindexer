...
extern float (*fvec_norm_L2sqr_ptr)(const float*, size_t);
}
...
inline float fvec_norm_L2sqr(const float* x, size_t d) {
    return distance_impl::fvec_norm_L2sqr_ptr(x, d);
}
...
/** Return the k nearest neighors of each of the nx vectors x among the ny
 *  vector y, w.r.t to max cosine metric.
 *
 * @param x    query vectors, size nx * d
 * @param y    database vectors, size ny * d
 * @param y_norms normalization coefficients for database vectors
 * @param res  result heap structure, which also provides k. Sorted on output
 */
void knn_cosine(
        const float* x,
...
/**  Return the k nearest neighors of each of the nx vectors x among the ny
 *  vector y, for the cosine metric
 *
 * @param x    query vectors, size nx * d
 * @param y    database vectors, size ny * d
 * @param y_norms normalization coefficients for database vectors
