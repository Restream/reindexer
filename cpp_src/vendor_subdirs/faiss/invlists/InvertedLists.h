...
struct InvertedLists {
    ...
    /**
     * get normalization coefficients for an inverted list (default implementation is nop)
     */
    virtual const float* get_norms(size_t list_no) const;
    ...
    /// release normalization coefficients returned by get_norms (default implementation is nop)
    virtual void release_norms(size_t list_no, const float* codes) const;
    ...
    /// @return a single normaliztion coefficient in an inverted list
    virtual const float* get_single_norm(size_t list_no, size_t offset) const;
    ...
    virtual size_t add_entries(
            ...
            const idx_t* ids,
            const uint8_t* code,
            const float* norm_coef = nullptr) = 0;
    ...
    virtual void update_entries(
            ...
            const idx_t* ids,
            const uint8_t* code,
            const float* norm_coef) = 0;

    virtual void resize(size_t list_no, size_t new_size) = 0;

    virtual void reset();

    virtual size_t allocated_mem_size() const;
    ...
    struct ScopedNorms {
        ...
        ScopedNorms(const InvertedLists* il, size_t list_no) noexcept
                : il(il),
                  norms(il->get_norms(list_no)),
                  list_no(list_no) {}

        ScopedNorms(const InvertedLists* il, size_t list_no, size_t offset) noexcept
                : il(il),
                  norms(il->get_single_norm(list_no, offset)),
                  list_no(list_no) {}
        ...
        ~ScopedNorms() {
            il->release_norms(list_no, norms);
        }
    };
};
...
struct ArrayInvertedLists : InvertedLists {
    using NormsContainerT = std::optional<std::vector<std::vector<float>>>;

    std::vector<std::vector<uint8_t>> codes; // binary codes, size nlist
    std::vector<std::vector<idx_t>> ids;     ///< Inverted lists for indexes
    NormsContainerT norms; ///< Normalization coefficients (optional)

    ArrayInvertedLists(size_t nlist, size_t code_size, bool with_norm_coefs);

    size_t list_size(size_t list_no) const override;
    const uint8_t* get_codes(size_t list_no) const override;
    const float* get_norms(size_t list_no) const override;
    const idx_t* get_ids(size_t list_no) const override;

    size_t add_entries(size_t list_no,
                       size_t n_entry,
                       const idx_t* ids,
                       const uint8_t* code,
                       const float* norm_coef) override;

    void update_entries(size_t list_no,
                        size_t offset,
                        size_t n_entry,
                        const idx_t* ids,
                        const uint8_t* code,
    ...
    size_t allocated_mem_size() const override;
...
struct ReadOnlyInvertedLists : InvertedLists {
    ...
    size_t add_entries(
            ...
            const idx_t* ids,
            const uint8_t* code,
            const float* norm = nullptr) override;
    ...
    void update_entries(
            ...
            const idx_t* ids,
            const uint8_t* code,
            const float* norm_coef = nullptr) override;
