...
const float *InvertedLists::get_norms(size_t /*list_no*/) const {
    return nullptr;
}
...
void InvertedLists::release_norms(size_t, const float*) const {}
...
const float* InvertedLists::get_single_norm(size_t list_no, size_t offset) const {
    assert(offset < list_size(list_no));
    const auto norms = get_norms(list_no);
    return norms ? norms + offset : nullptr;
}

size_t InvertedLists::add_entry(size_t list_no,
                                idx_t theid,
                                const uint8_t* code,
                                void* /*inverted_list_context*/,
                                const float* norm) {
    return add_entries(list_no, 1, &theid, code, norm);
}
...
void InvertedLists::update_entry(
    ...
    update_entries(list_no, offset, 1, &id, code, norm_coef);
}
...
size_t InvertedLists::allocated_mem_size() const {
    FAISS_THROW_MSG("allocated_mem_size() not implemented for this inverted list type");
}
...
ArrayInvertedLists::ArrayInvertedLists(size_t nlist, size_t code_size, bool with_norm_coefs)
...
size_t ArrayInvertedLists::add_entries(size_t list_no,
                                       size_t n_entry,
                                       const idx_t* ids_in,
                                       const uint8_t* code,
                                       const float* norm_coef) {
    ...
    if (norm_coef && norms.has_value()) {
        auto& normRef = (*norms)[list_no];
        normRef.resize(o + n_entry);
        memcpy(&(normRef[o]), norm_coef, sizeof(float) * n_entry);
    }
    return o;
}
...
size_t ArrayInvertedLists::allocated_mem_size() const {
    size_t ret = 0;
    ret += codes.capacity() * sizeof(std::vector<uint8_t>);
    for (auto& c : codes) {
        ret += c.capacity() * sizeof(uint8_t);
    }
    ret += ids.capacity() * sizeof(std::vector<idx_t>);
    for (auto& i : ids) {
        ret += i.capacity() * sizeof(idx_t);
    }
    if (norms.has_value()) {
        ret += norms->capacity() * sizeof(std::vector<float>);
        for (auto& n : *norms) {
            ret += n.capacity() * sizeof(float);
        }
    }
    return ret;
}
...
const float *ArrayInvertedLists::get_norms(size_t list_no) const {
    if (norms.has_value()) {
        return (*norms)[list_no].data();
    } else {
        return nullptr;
    }
}
...
void ArrayInvertedLists::update_entries(
        ...
        const float* norm_coefs_in) {
    assert(list_no < nlist);
    assert(n_entry + offset <= ids[list_no].size());
    memcpy(&ids[list_no][offset], ids_in, sizeof(ids_in[0]) * n_entry);
    if (norm_coefs_in && norms.has_value()) {
        memcpy(&(*norms)[list_no][offset], norm_coefs_in, sizeof(norm_coefs_in[0]) * n_entry);
    }
...
void ArrayInvertedLists::permute_invlists(const idx_t* map) {
    ...
    NormsContainerT new_norms;
    if (norms.has_value()) {
        new_norms.emplace();
        new_norms->resize(nlist);
    }
    ...
    for (size_t i = 0; i < nlist; i++) {
        ...
        if (norms.has_value()) {
            std::swap((*new_norms)[i], (*norms)[o]);
        }
    }
    std::swap(codes, new_codes);
    std::swap(ids, new_ids);
    std::swap(norms, new_norms);
}
