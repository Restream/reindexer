...
using ScopedNorms = InvertedLists::ScopedNorms;
            ...
            if (res != hashtable.end()) {
                ...
                if (offset < last) {
                    ...
                    invlists->update_entry(
                            ...
                            ScopedNorms(invlists, list_no, last).get());
    ...
    for (size_t i = 0; i < n; i++) {
        ...
        { // remove old one
            ...
            if (ofs != l - 1) { // move l - 1 to ofs
                ...
                invlists->update_entry(
                        il, ofs, id2, invlists->get_single_code(il, l - 1), invlists->get_single_norm(il, l - 1));
            }
...
size_t DirectMap::allocated_mem_size() const noexcept {
    constexpr static size_t kElemSizeOverhead = 256;
    size_t ret = 0;
    ret += array.capacity() * sizeof(idx_t);
    // Totally unprecise calculation for unordered map, but it should be fine for our purposes
    ret += hashtable.size() * (kElemSizeOverhead + sizeof(idx_t) + sizeof(idx_t));
    return ret;
}

size_t DirectMap::unique_ids_count() const noexcept {
    switch(type) {
    case Array:
        return array.size();
    case Hashtable:
