...
IndexFlat::IndexFlat(idx_t d, MetricType metric, bool is_cosine)
        : IndexFlatCodes(sizeof(float) * d, d, metric, is_cosine) {}
...
void IndexFlat::search(
    ...
    if (metric_type == METRIC_INNER_PRODUCT) {
        ...
        if (is_cosine) {
            knn_cosine(x, get_xb(), norm_coefs.data(), d, n, ntotal, &res, sel);
        } else {
...
void IndexFlat::reconstruct(idx_t key, float* recons) const noexcept {
    memcpy(recons, &(codes[key * code_size]), code_size);
}

reindexer::ConstFloatVectorView IndexFlat::getView(idx_t key) const noexcept {
    return reindexer::ConstFloatVectorView{std::span(reinterpret_cast<const float*>(&(codes[key * code_size])), d)};
}
...
size_t IndexFlat::allocated_mem_size() const {
    return IndexFlatCodes::allocated_mem_size();
}
