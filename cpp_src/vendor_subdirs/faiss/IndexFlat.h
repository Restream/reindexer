...
struct IndexFlat : IndexFlatCodes {
    ...
    void reconstruct(idx_t key, float* recons) const noexcept override;
    reindexer::ConstFloatVectorView getView(idx_t key) const noexcept;
    ...
    const float* get_xb_norms() const {
        return norm_coefs.empty() ? nullptr : reinterpret_cast<const float*>(norm_coefs.data());
    }
...
struct IndexFlatCosine : IndexFlat {
    explicit IndexFlatCosine(idx_t d) : IndexFlat(d, METRIC_INNER_PRODUCT, true) {}
    IndexFlatCosine() {}
};
