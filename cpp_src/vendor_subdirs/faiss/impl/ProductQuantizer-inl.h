...
inline PQEncoder8::PQEncoder8(uint8_t* code, [[maybe_unused]] int nbits) : code(code) {
    assert(8 == nbits);
}
...
inline PQEncoder16::PQEncoder16(uint8_t* code, [[maybe_unused]] int nbits)
        : code((uint16_t*)code) {
    assert(16 == nbits);
}
...
inline PQDecoder8::PQDecoder8(const uint8_t* code, [[maybe_unused]] int nbits_in) : code(code) {
    assert(8 == nbits_in);
}
...
inline PQDecoder16::PQDecoder16(const uint8_t* code, [[maybe_unused]] int nbits_in)
        : code((uint16_t*)code) {
    assert(16 == nbits_in);
}
