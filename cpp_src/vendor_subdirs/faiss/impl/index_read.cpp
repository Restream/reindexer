...
InvertedLists* read_InvertedLists(IOReader* f, int io_flags) {
    ...
    if (h == fourcc("il00")) {
        ...
        return nullptr;
    } else if (h == fourcc("ilas") && (io_flags & IO_FLAG_SKIP_IVF_DATA)) {
        FAISS_THROW_MSG("unsuported list type 'ilas' with IO_FLAG_SKIP_IVF_DATA option");
    }

    if ((h == fourcc("ilas") || h == fourcc("ilar")) && !(io_flags & IO_FLAG_SKIP_IVF_DATA)) {
        size_t nlist = 0, code_size = 0;
        bool with_norm_coefs = false;
        READ1(nlist);
        READ1(code_size);
        READ1(with_norm_coefs);
        auto ails = new ArrayInvertedLists(nlist, code_size, with_norm_coefs);
        ...
        if (h == fourcc("ilas")) {
            for (size_t i = 0; i < ails->nlist; i++) {
                const size_t n = ails->ids[i].size();
                ails->codes[i].resize(n * code_size);
                if (with_norm_coefs) {
                    (*ails->norms)[i].resize(n);
                }
                uint8_t* codesPtr = ails->codes[i].data();
                for (size_t j = 0; j < n; ++j) {
                    ails->ids[i][j] = f->ReadPKEncodedData(codesPtr);
                    codesPtr += code_size;
                    if (with_norm_coefs) {
                        READ1((*ails->norms)[i][j]);
                    }
                }
            ...
            for (size_t i = 0; i < ails->nlist; i++) {
                ...
                if (n > 0) {
                    ...
                    if (ails->norms.has_value()) {
                        READANDCHECK((*ails->norms)[i].data(), n);
                    }
                }
            }
        } else {
...
void read_ivf_header(IndexIVF* ivf,
        IOReader* f,
...
static ArrayInvertedLists* set_array_invlist(
    ...
    ArrayInvertedLists* ail =
            new ArrayInvertedLists(ivf->nlist, ivf->code_size, false);
...
static constexpr int read_old_fmt_hack = 0;
...
Index* read_index(IOReader* f, int io_flags) {
    ...
    if (h == fourcc("null")) {
        ...
        IndexIVFFlat* ivfl = new IndexIVFFlat();
        read_ivf_header(ivfl, f, nullptr);
        ivfl->code_size = ivfl->d * sizeof(float);
        read_InvertedLists(ivfl, f, io_flags);
        if (ivfl->direct_map.type == DirectMap::Hashtable && ivfl->direct_map.hashtable.empty()) {
            // Recreate map if it's empty
            ivfl->direct_map.set_type(DirectMap::NoMap, nullptr, 0);
            ivfl->direct_map.set_type(DirectMap::Hashtable, ivfl->invlists, ivfl->ntotal);
        }
        idx = ivfl;
