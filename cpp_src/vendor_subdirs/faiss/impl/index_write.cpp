...
void write_InvertedLists(const InvertedLists* ils, const std::atomic_int32_t& cancel, bool skipCodesDump, IOWriter* f) {
    if (ils == nullptr) {
        ...
        uint32_t h = skipCodesDump ? fourcc("ilas") : fourcc("ilar");
        WRITE1(h);
        WRITE1(ails->nlist);
        WRITE1(ails->code_size);
        const bool with_norm_coefs = ails->norms.has_value();
        WRITE1(with_norm_coefs);
        ...
        if (skipCodesDump) {
            for (size_t i = 0; i < ails->nlist; i++) {
                const auto& ids_vec = ails->ids[i];
                for (size_t j = 0, ids_cnt = ids_vec.size(); j < ids_cnt; ++j) {
                    f->AppendPKByID(ids_vec[j]);
                    if (with_norm_coefs) {
                        WRITE1((*ails->norms)[i][j]);
                    }
                }
                if (cancel.load(std::memory_order_relaxed)) {
                    FAISS_THROW_MSG("Inverted lists saving was canceled");
            ...
            for (size_t i = 0; i < ails->nlist; i++) {
                ...
                if (n > 0) {
                    ...
                    if (with_norm_coefs) {
                        WRITEANDCHECK((*ails->norms)[i].data(), n);
                    }
        ...
        assert(!skipCodesDump); // Not implemented
...
static void write_direct_map(const DirectMap* dm, IOWriter* f, bool skipMapData) {
    char maintain_direct_map =
            (char)dm->type; // for backwards compatibility with bool
    WRITE1(maintain_direct_map);
    if (skipMapData) {
        const std::vector<idx_t> empty;
        WRITEVECTOR(empty);
    } else {
    ...
    if (dm->type == DirectMap::Hashtable) {
        ...
        if (!skipMapData) { // Do not write map - it will be recreated after index data reading
            v.resize(map.size());
...
static void write_ivf_header(const IndexIVF* ivf, IOWriter* f, const std::atomic_int32_t& cancel, bool skipMapData = false) {
    write_index_header(ivf, f);
    WRITE1(ivf->nlist);
    WRITE1(ivf->nprobe);
    // subclasses write by_residual (some of them support only one setting of
    // by_residual).
    constexpr bool skipCodesDump = false;
    write_index(ivf->quantizer, f, cancel, skipCodesDump);
    write_direct_map(&ivf->direct_map, f, skipMapData);
}

void write_index(const Index* idx, IOWriter* f, const std::atomic_int32_t& cancel, bool skipCodesDump, int io_flags) {
    if (idx == nullptr) {
        ...
        write_ivf_header(ivaqfs, f, cancel);
        ...
        write_InvertedLists(ivaqfs->invlists, cancel, skipCodesDump, f);
    } else if (
        ...
        WRITE1(h);
        write_index_header(idx, f);
        write_index(idxp_2->q1.quantizer, f, cancel, skipCodesDump);
        ...
        WRITE1(h);
        write_ivf_header(ivfl, f, cancel);
        {
        ...
        write_InvertedLists(ivfl->invlists, cancel, skipCodesDump, f);
    } else if (
        ...
        WRITE1(h);
        write_ivf_header(ivsc, f, cancel);
        write_ScalarQuantizer(&ivsc->sq, f);
        WRITE1(ivsc->code_size);
        WRITE1(ivsc->by_residual);
        write_InvertedLists(ivsc->invlists, cancel, skipCodesDump, f);
        ...
        write_InvertedLists(iva->invlists, cancel, skipCodesDump, f);
    } else if (
        ...
        WRITE1(h);
        write_ivf_header(ivsp, f, cancel);
        write_VectorTransform(ivsp->vt, f);
        WRITE1(ivsp->nbit);
        WRITE1(ivsp->period);
        WRITE1(ivsp->threshold_type);
        WRITEVECTOR(ivsp->trained);
        write_InvertedLists(ivsp->invlists, cancel, skipCodesDump, f);
        ...
        WRITE1(h);
        write_ivf_header(ivpq, f, cancel);
        WRITE1(ivpq->by_residual);
        WRITE1(ivpq->code_size);
        write_ProductQuantizer(&ivpq->pq, f);
        write_InvertedLists(ivpq->invlists, cancel, skipCodesDump, f);
        if (ivfpqr) {
        ...
        write_index(indep->quantizer, f, cancel, skipCodesDump);
        bool has_vt = indep->vt != nullptr;
        WRITE1(has_vt);
        if (has_vt) {
            write_VectorTransform(indep->vt, f);
        }
        write_index(indep->index_ivf, f, cancel, skipCodesDump);
        ...
        write_index(ixpt->index, f, cancel, skipCodesDump);
    } else if (
        ...
        write_index(idxrf->base_index, f, cancel, skipCodesDump);
        write_index(idxrf->refine_index, f, cancel, skipCodesDump);
        ...
        write_index(idxmap->index, f, cancel, skipCodesDump);
        ...
        if (io_flags & IO_FLAG_SKIP_STORAGE) {
            ...
            write_index(idxhnsw->storage, f, cancel, skipCodesDump);
        }
        ...
        write_index(idxnsg->storage, f, cancel, skipCodesDump);
    } else if (
        ...
        write_index(idxnnd->storage, f, cancel, skipCodesDump);
    } else if (
        ...
        WRITE1(h);
        write_ivf_header(ivpq_2, f, cancel);
        ...
        write_InvertedLists(ivpq_2->invlists, cancel, skipCodesDump, f);
    } else if (
        ...
        write_index(imm->index, f, cancel, skipCodesDump);
    } else if (
        ...
        write_index(imm_2->index, f, cancel, skipCodesDump);
    } else {
        FAISS_THROW_MSG("don't know how to serialize this type of index");
    }
}

void write_index(const Index* idx, FILE* f, const std::atomic_int32_t& cancel, bool skipCodesDump, int io_flags) {
    FileIOWriter writer(f);
    write_index(idx, &writer, cancel, skipCodesDump, io_flags);
}

void write_index(const Index* idx, const char* fname, const std::atomic_int32_t& cancel, bool skipCodesDump, int io_flags) {
    FileIOWriter writer(fname);
    write_index(idx, &writer, cancel, skipCodesDump, io_flags);
}
...
static void write_binary_ivf_header(const IndexBinaryIVF* ivf, IOWriter* f) {
    ...
    constexpr bool skipMapData = false;
    write_direct_map(&ivf->direct_map, f, skipMapData);
}
...
void write_index_binary(const IndexBinary* idx, IOWriter* f) {
    ...
    if (const IndexBinaryFlat* idxf =
        ...
        write_InvertedLists(ivf->invlists, cancel, false, f);
    } else if (
        ...
        write_index(idxff->index, f, cancel, false);
    } else if (
