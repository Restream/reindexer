...
bool matchLikePattern(string_view utf8Str, string_view utf8Pattern) {
	...
	while (pIt != utf8Pattern.end() && sIt != utf8Str.end()) {
	...
	while (pIt != utf8Pattern.end() && sIt != utf8Str.end()) {
		...
		while (tmpPIt != utf8Pattern.end()) {
			...
			if (tmpSIt == utf8Str.end()) return false;
			if (ToLower(pCh) != ToLower(utf8::unchecked::next(tmpSIt)) && pCh != anyChar) {
				utf8::unchecked::next(sIt);
				break;
			}
		}
		if (tmpPIt == utf8Pattern.end()) {
			sIt = tmpSIt;
			pIt = tmpPIt;
		}
	}

	while (pIt != utf8Pattern.end()) {
		if (utf8::unchecked::next(pIt) != wildChar) return false;
		haveWildChar = true;
	}

	if (!haveWildChar && sIt != utf8Str.end()) return false;

	for (pIt = utf8Pattern.end(), sIt = utf8Str.end(); pIt != utf8Pattern.begin() && sIt != utf8Str.begin();) {
