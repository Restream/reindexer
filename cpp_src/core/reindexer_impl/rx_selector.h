...
public:
	...
	static void DoSelect(const impl::Query& q, std::optional<impl::Query>& queryCopy, LocalQueryResults& result, LockerType& locks,
						 FtFunctionsHolder& func, const RdxContext& ctx);

	static void DoPreSelectForUpdateDelete(const impl::Query& q, std::optional<impl::Query>& queryCopy, LocalQueryResults& result,
										   NsLockerW& locks, FloatVectorsHolderMap* fvHolder, const RdxContext& ctx);

private:
	template <typename LockerT>
	static std::vector<SubQueryExplain> preselectSubQueries(impl::Query& mainQuery, std::vector<LocalQueryResults>& queryResultsHolder,
															LockerT&, FtFunctionsHolder&, const RdxContext&);
	template <typename LockerT>
	static bool selectSubQuery(const impl::Query& subQuery, const impl::Query& mainQuery, LockerT&, FtFunctionsHolder&,
							   std::vector<SubQueryExplain>&, const RdxContext&);
	template <typename LockerT>
	static VariantArray selectSubQuery(const impl::Query& subQuery, const impl::Query& mainQuery, LockerT&, LocalQueryResults&,
									   FtFunctionsHolder&, std::variant<std::string, size_t> fieldOrKeys, std::vector<SubQueryExplain>&,
									   const RdxContext&);

	template <typename LockerType>
	static void preselectSubQuriesMain(const impl::Query& q, std::optional<impl::Query>& queryCopy, LockerType& locks,
