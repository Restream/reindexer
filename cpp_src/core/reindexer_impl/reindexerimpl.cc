...
static void securityCheck(const impl::Query& query, const RdxContext& rdxCtx) {
...
Error ReindexerImpl::modifyQ(const impl::Query& query, LocalQueryResults& result, const RdxContext& rdxCtx,
							 void (NamespaceImpl::*fn)(LocalQueryResults&, UpdatesContainer&, const impl::Query&, const NsContext&,
	...
	try {
		...
		const std::string& nsName = query.NsName();
		{
			...
			std::optional<impl::Query> queryCopy = embedQuery(query, rdxCtx, precomputedValues);
			...
			{
				const impl::Query& q = queryCopy ? *queryCopy : query;
				...
				query.WalkNested(false, false, true, [this, &locks, &rdxCtx](const impl::Query& q) {
...
Error ReindexerImpl::Update(const impl::Query& query, LocalQueryResults& result, const RdxContext& rdxCtx) {
...
Error ReindexerImpl::Delete(const impl::Query& query, LocalQueryResults& result, const RdxContext& rdxCtx) {
	return modifyQ<QueryType::QueryDelete>(query, result, rdxCtx, &NamespaceImpl::doDelete);
}

Error ReindexerImpl::Select(const impl::Query& q, LocalQueryResults& result, const RdxContext& rdxCtx) {
	try {
		...
		q.WalkNested(false, true, true, [this, &refs](const impl::Query& q) {
	...
	if (!embedHolder.empty()) {
		...
		for (const auto& embed : embedHolder) {
			try {
				...
				[[maybe_unused]] auto inserted = queryCopy->template EmplaceQueryEntry<KnnQueryEntry>(embed.entryId, embed.qe.FieldName(),
																									  products.front(), embed.qe.Params());
...
template <concepts::OneOf<impl::Query, impl::JoinedQuery> Q>
void ReindexerImpl::embedNestedQueries(const impl::Query& q, const std::vector<Q>& nestedQueries,
									   std::invocable<impl::Query&, size_t, Q&&> auto replacer, const RdxContext& ctx,
									   std::optional<impl::Query>& queryCopy, functions::PrecomputedValues& precomputedValues) {
...
std::optional<impl::Query> ReindexerImpl::embedQuery(const impl::Query& q, const RdxContext& ctx,
													 functions::PrecomputedValues& precomputedValues) {
	std::optional<impl::Query> queryCopy{embedKNNQueries(q, ctx)};
	OptimizeFunctionEntries(q, queryCopy, precomputedValues);
	const impl::Query& query{queryCopy ? queryCopy.value() : q};
	embedNestedQueries(
		q, query.SubQueries(), [](impl::Query& qr, size_t i, impl::Query&& queryN) { qr.ReplaceSubQuery(i, std::move(queryN)); }, ctx,
		queryCopy, precomputedValues);
	embedNestedQueries(
		q, query.JoinQueries(), [](impl::Query& qr, size_t i, impl::JoinedQuery&& queryN) { qr.ReplaceJoinQuery(i, std::move(queryN)); },
		ctx, queryCopy, precomputedValues);
	embedNestedQueries(
		q, query.MergeQueries(), [](impl::Query& qr, size_t i, impl::JoinedQuery&& queryN) { qr.ReplaceMergeQuery(i, std::move(queryN)); },
		ctx, queryCopy, precomputedValues);
	return queryCopy;
}
...
Error ReindexerImpl::initSystemNamespaces() {
	...
	auto err = Select(impl::Query(kConfigNamespace), results, RdxContext());
	if (!err.ok()) {
		return err;
	}
	...
	err = Select(impl::Query(kConfigNamespace), results, RdxContext());
	if (!err.ok()) {
		return err;
	}
...
ReindexerImpl::FilterNsNamesT ReindexerImpl::detectFilterNsNames(const impl::Query& q) {
	FilterNsNamesT res;
...
void ReindexerImpl::onProfilingConfigLoad() {
	...
	RdxContext ctx;
	auto err = Delete(impl::Query(kMemStatsNamespace), qr2, ctx);
	err = Delete(impl::Query(kQueriesPerfStatsNamespace), qr3, ctx);
	err = Delete(impl::Query(kPerfStatsNamespace), qr1, ctx);
	(void)err;	// ignore
}
...
Error ReindexerImpl::GetProtobufSchema(WrSerializer& ser, std::vector<std::string>& namespaces) {
	...
	for (auto& ns : nses) {
		...
		{
			impl::Query q(ns.nsName);
			q.Limit(0);
			status = Select(q, qr, RdxContext());
		}
		if (!status.ok()) {
			return status;
		}
