...
public:
	...
	Error Update(const impl::Query& query, LocalQueryResults& result, const RdxContext& ctx);
	...
	Error Delete(const impl::Query& query, LocalQueryResults& result, const RdxContext& ctx);
	Error Select(const impl::Query& query, LocalQueryResults& result, const RdxContext& ctx);
...
private:
	...
	FilterNsNamesT detectFilterNsNames(const impl::Query& q);
	...
	template <concepts::OneOf<impl::Query, impl::JoinedQuery> Q>
	void embedNestedQueries(const impl::Query& q, const std::vector<Q>& nestedQueries,
	...
	std::optional<impl::Query> embedQuery(const impl::Query& query, const RdxContext& ctx, functions::PrecomputedValues& precomputedValues);

	template <QueryType TP>
	Error modifyQ(const impl::Query& query, LocalQueryResults& result, const RdxContext& rdxCtx,
