...
using str_map_rebalance_params = extendible_hash_map_rebalance_params<(1 << 20), 12>;

template <typename T1>
class [[nodiscard]] unordered_str_map
	: public extendible_hash_map<
		  tsl::sparse_map<key_string, T1, hash_key_string, equal_key_string, std::allocator<std::pair<key_string, T1>>,
						  tsl::sh::power_of_two_growth_policy<2>, tsl::sh::exception_safety::basic, tsl::sh::sparsity::low>,
		  str_map_rebalance_params, key_string, T1, hash_key_string, equal_key_string> {
...
public:
	...
	unordered_str_map(const CollateOpts& opts) : base_hash_map(hash_key_string(CollateMode(opts.mode)), equal_key_string(opts)) {}
	...
	template <typename deep_cleaner>
	void erase(iterator pos, const deep_cleaner& deep_clean) {
		deep_clean(*pos);
		base_hash_map::erase(pos);
	}
};
...
using number_map_rebalance_params = extendible_hash_map_rebalance_params<(1 << 21), 11>;

template <typename K, typename T>
class [[nodiscard]] unordered_number_map
	: public extendible_hash_map<
		  tsl::sparse_map<K, T, hash_int<K>, std::equal_to<K>, std::allocator<std::pair<K, T>>, tsl::sh::power_of_two_growth_policy<2>,
						  tsl::sh::exception_safety::basic, tsl::sh::sparsity::low>,
		  number_map_rebalance_params, K, T, hash_int<K>, std::equal_to<K>> {
	using base_hash_map = extendible_hash_map<
		tsl::sparse_map<K, T, hash_int<K>, std::equal_to<K>, std::allocator<std::pair<K, T>>, tsl::sh::power_of_two_growth_policy<2>,
						tsl::sh::exception_safety::basic, tsl::sh::sparsity::low>,
		number_map_rebalance_params, K, T, hash_int<K>, std::equal_to<K>>;
...
using uuid_map_rebalance_params = extendible_hash_map_rebalance_params<(1 << 21), 11>;

template <typename T>
class [[nodiscard]] unordered_uuid_map
	: public extendible_hash_map<
		  tsl::sparse_map<Uuid, T, hash_uuid, std::equal_to<Uuid>, std::allocator<std::pair<Uuid, T>>,
						  tsl::sh::power_of_two_growth_policy<2>, tsl::sh::exception_safety::basic, tsl::sh::sparsity::low>,
		  uuid_map_rebalance_params, Uuid, T, hash_uuid, std::equal_to<Uuid>> {
	using base_hash_map = extendible_hash_map<
		tsl::sparse_map<Uuid, T, hash_uuid, std::equal_to<Uuid>, std::allocator<std::pair<Uuid, T>>, tsl::sh::power_of_two_growth_policy<2>,
						tsl::sh::exception_safety::basic, tsl::sh::sparsity::low>,
		uuid_map_rebalance_params, Uuid, T, hash_uuid, std::equal_to<Uuid>>;
