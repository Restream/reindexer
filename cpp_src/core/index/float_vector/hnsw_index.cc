...
auto HnswIndexBase<Map>::search(const float* key, const ParamsT& params) const {
...
SelectKeyResult HnswIndexBase<Map>::select(ConstFloatVectorView key, const KnnSearchParams& params, KnnCtx& ctx) const {
	...
	auto knnRes = std::is_same_v<Map<FloatType>, hnswlib::BruteforceSearch<FloatType>> ? search(keyData, params.BruteForce())
