...
public:
	...
	[[nodiscard]] const DummyMutex& operator!() const& noexcept { return *this; }
...
public:
	template <typename Mtx>
	struct RX_SCOPED_CAPABILITY UniqueLock : public reindexer::unique_lock<Mtx> {
	...
	template <typename Mtx>
	struct RX_SCOPED_CAPABILITY LockGuard : public reindexer::lock_guard<Mtx> {
		explicit LockGuard(Mtx& mtx) RX_ACQUIRE(mtx) : reindexer::lock_guard<Mtx>{mtx} {}
		~LockGuard() RX_RELEASE() = default;
	};

	template <typename Mtx>
	class RX_SCOPED_CAPABILITY SharedLock : public reindexer::shared_lock<Mtx> {
...
template <>
struct RegularLocker::Concurrent<ExpectConcurrentUpdates::Yes> {
	template <typename Mtx>
	struct RX_SCOPED_CAPABILITY SharedLock : public RegularLocker::SharedLock<Mtx> {
...
template <>
struct RegularLocker::Concurrent<ExpectConcurrentUpdates::No> {
	template <typename Mtx>
	struct SharedLock {
	...
	tableint addPoint(const void* data_point, labeltype label, int level)
		...
		tableint cur_c = 0;
		{
			...
			if (search != label_lookup_.end()) {
				...
				updatePoint<LockerT>(data_point, existingInternalId, 1.0);
