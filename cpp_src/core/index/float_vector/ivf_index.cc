...
static void search(base_idset& idset, const IVFSearchArgsKnn& args, const auto& map, const auto& filter, const auto& sortSameDist,
	...
	map->search(1, args.keyData, args.k, args.dists, args.ids, args.params);
	for (size_t i = 0; i < args.k; ++i) {
		const auto id = args.ids[i];
		if (id < 0 || !filter(i)) {
			break;
		}
		sortSameDist(i);
...
static void search(base_idset& idset, const IVFSearchArgsRange& args, const auto& map, auto& dists, auto& ids, VectorMetric metric,
...
SelectKeyResult IvfIndex::select(ConstFloatVectorView key, const KnnSearchParams& p, KnnCtx& ctx) const {
	...
	const auto sortSameDist = [&](size_t i) {
		bool newDist{false};
		const auto distsData = dists.data();
		switch (metric_) {
			case VectorMetric::L2:
	...
	const auto prepareId = [this](size_t i) { return n2RowId_[i]; };
	const auto empty = [](size_t) { return true; };
	const bool withSort = *ctx.NeedSort();
	const bool withIVF = bool(map_);

	if (k > 0) {
		const auto filter = [this, distsData = dists.data(), radius](size_t i) {
			switch (metric_) {
				case VectorMetric::L2:
		...
		IVFSearchArgsKnn args{.keyData = keyData, .k = k, .dists = dists.data(), .ids = ids.data(), .params = &param};
		...
		IVFSearchArgsRange args{.keyData = keyData, .radius = *radius, .result = &result, .params = &param};
		if (withIVF && withSort) {
			search(idset, args, map_, dists, ids, metric_, sortSameDist, std::identity{});
		} else if (withIVF && !withSort) {
			search(idset, args, map_, dists, ids, metric_, empty, std::identity{});
		} else if (!withIVF && withSort) {
			search(idset, args, space_, dists, ids, metric_, sortSameDist, prepareId);
		} else if (!withIVF && !withSort) {
			search(idset, args, space_, dists, ids, metric_, empty, prepareId);
		}
	} else {
		throw_as_assert;
	}
