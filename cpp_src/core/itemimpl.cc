...
namespace {
bool isFieldEmpty(ItemImpl& item, const Embedder& embedder) {
	Variant vecVar = item.GetField(item.GetPayload().Type().FieldByName(embedder.FieldName()));
	assertrx_throw(vecVar.Type().Is<KeyValueType::FloatVector>());
	auto vec = ConstFloatVectorView(vecVar);
	return vec.IsEmpty();
}

bool checkEmbedderAllowed(ItemImpl& item, const Embedder& embedder) {
...
void ItemImpl::Embed(const RdxContext& ctx) {
	try {
		...
		for (int field = 1, numFields = payloadType_.NumFields(); field < numFields; ++field) {
			if (!payloadType_.Field(field).Embedder()) {
				continue;
			}
			...
			VariantArray krs;
			krs.emplace_back(vect);

			SetField(pl.Type().FieldByName(embedder->FieldName()), krs, NeedCreate_False);
		}
		unsafe_ = unsafe;
	} catch (Error& err) {
