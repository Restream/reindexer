...
void JoinedFieldIterator::updateOffset() noexcept {
	currField_ = -1;
	if (order_ == joinRes_->GetJoinItemsProcessorsCount()) {
		return;
	}
...
int JoinedFieldIterator::ItemsCount() const noexcept {
	...
	assertrx(order_ < joinRes_->GetJoinItemsProcessorsCount());
...
JoinedFieldIterator ItemIterator::at(uint8_t joinedField) const {
	...
	assertrx_throw(joinedField < joinRes_->GetJoinItemsProcessorsCount());
...
JoinedFieldIterator ItemIterator::end() const noexcept {
	...
	return JoinedFieldIterator(joinRes_, it->second, joinRes_->GetJoinItemsProcessorsCount());
}
...
void NamespaceResults::Insert(IdType rowid, uint32_t fieldIdx, LocalQueryResults&& qr) {
	assertrx_throw(fieldIdx < joinItemsProcessorsCount_);
	ItemOffsets& offsets = offsets_[rowid];
	if (offsets.empty()) {
		offsets.reserve(joinItemsProcessorsCount_);
	}
