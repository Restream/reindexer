...
public:
	ItemsProcessor(JoinType joinType, NamespaceImpl::Ptr leftNs, NamespaceImpl::Ptr rightNs, CacheRes&& joinRes, impl::Query&& itemQuery,
				   FieldsFilter fieldsFilter, LocalQueryResults& result, const impl::JoinedQuery& joinQuery,
				   PreSelectExecuteCtx&& preSelCtx, uint32_t joinedFieldIdx, FtFunctionsHolder& selectFunctions, bool inTransaction,
				   int64_t lastUpdateTime, SetLimit0ForChangeJoin limit0, const RdxContext& rdxCtx)
		: joinType_(joinType),
		  called_(0),
		  matched_(0),
	...
	static std::vector<ItemsProcessor> BuildForQuery(const impl::Query& q, LocalQueryResults& result, Locker& locks,
													 FtFunctionsHolder& func, std::vector<QueryResultsContext>*,
													 IsModifyQuery isModifyQuery, const RdxContext& ctx);
	...
	const impl::JoinedQuery& JoinQuery() const noexcept { return joinQuery_; }
...
private:
	...
	void selectFromRightNs(LocalQueryResults& joinItemR, const impl::Query&, FloatVectorsHolderMap*, bool& found, bool& matchedAtLeastOnce);
	void selectFromPreSelectValues(LocalQueryResults& joinItemR, const impl::Query&, bool& found, bool& matchedAtLeastOnce) const;

	static StoredValuesOptimizationStatus isPreSelectValuesOptimizationEnabled(const impl::Query& jItemQ, const NamespaceImpl::Ptr& jns,
	...
	CacheRes joinRes_;
	impl::Query itemQuery_;
	FieldsFilter fieldsFilter_;
	LocalQueryResults& result_;
	const impl::JoinedQuery& joinQuery_;
