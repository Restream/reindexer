...
void JoinedSelector::AppendSelectIteratorOfJoinIndexData(SelectIteratorContainer& iterators, int* maxIterations, unsigned sortId,
	...
	for (size_t i = 0; i < joinQuery_.joinEntries_.size(); ++i) {
		...
		if (leftIndex->Opts().IsSparse()) {
			continue;
		}

		const VariantArray values = std::visit(
			overloaded{[&](const IdSet& preselected) {
						   return readValuesOfRightNsFrom(
							   preselected,
							   [this](IdType rowId) noexcept { return ConstPayload{rightNs_->payloadType_, rightNs_->items_[rowId]}; },
