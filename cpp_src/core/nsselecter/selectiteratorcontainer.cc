...
SelectKeyResults SelectIteratorContainer::processQueryEntry(const QueryEntry& qe, const NamespaceImpl& ns, StrictMode strictMode) {
	if (!qe.HaveEmptyField()) {
		...
		if (qe.Condition() == CondEmpty) {
			return SelectKeyResult{{SingleSelectKeyResult{IdType::Zero(), IdType::FromNumber(ns.items_.size())}}};
		} else {
			return SelectKeyResult{{SingleSelectKeyResult{IdType::Zero(), IdType::Zero()}}};
		}
	} else {
		throw Error(
			errStrictMode,
...
void SelectIteratorContainer::processJoinEntry(const JoinQueryEntry& jqe, OpType op) {
	...
	assertrx_throw(op != OpOr || lastAppendedOrClosed() != this->end());
...
void SelectIteratorContainer::bindFieldEqualPositions(const NamespaceImpl& ns, const EqualPositions& eqPos, const QueryEntries& queries,
	...
	for (size_t i = 0, size = eqPos.size(); i < size; ++i) {
		...
		if constexpr (std::is_same_v<EqCompT, EqualPositionComparator>) {
			if (qe.Fields()[0] == IndexValueType::SetByJsonPath) {
				const auto& fieldPath = qe.Fields().getFieldsPath(0);
				size_t sz = std::visit([](auto& p) { return p.size(); }, fieldPath);
				if (sz == 0) {
					containsNonExistingFields = true;
					break;
				}
	...
	if (containsNonExistingFields) {
		// Using empty IdSet for non-exisiting fields instead of actual comparator
		std::ignore = Append<SelectIterator>(OpAnd, SelectKeyResult{}, IsDistinct_False, std::string("Empty equal position"),
											 IndexValueType::NotSet, ForcedFirst_False);
...
SelectKeyResult SelectIteratorContainer::processKnnQueryEntry(const QueryPreprocessor& qPreproc, const KnnQueryEntry& qe,
															  const NamespaceImpl& ns, RanksHolder::Ptr& ranks, const RdxContext& rdxCtx) {
	assertrx_throw(ctx_);
	...
	if (streamingKnnMode_) {
		...
		const size_t limit = qPreproc.Count();
		const size_t needed =
			size_t(offset) + size_t(limit);	 // 'limit' is huge (kDefaultLimit) when unbounded -> stop only on exhausted/cap

		size_t ef = qe.Params().Hnsw().Ef();
		if (ef == 1) {
			const size_t itemsCount = ns.itemsCount();
			const int maxItersRaw =
				qPreproc.CalculateMaxIterationsForStreamingKnn(ctx_->inTransaction, ctx_->sortingContext.enableSortOrders, rdxCtx);
			const size_t maxIters = (maxItersRaw > 0) ? size_t(maxItersRaw) : itemsCount;
			ef = StreamingKnnEstimator::EstimateEf(maxIters, itemsCount, offset, limit);
		}
		...
		containRanked |= queries.Visit(
			...
			[&](const KnnQueryEntry& qe) {
				...
				if (queryRankType == QueryRankType::Hybrid) {
					...
					[[maybe_unused]] auto inserted =
						Append<SelectIterator>(op, processKnnQueryEntry(qPreproc, qe, ns, ranks, rdxCtx), IsDistinct_False, qe.FieldName(),
											   qe.IndexNo(), ForcedFirst_True);
					assertrx_throw(inserted == 1);
					if (!streamingKnnMode_) {
						lastAppendedOrClosed()->Value<SelectIterator>().SetUnsorted();
					}
...
template bool SelectIteratorContainer::checkIfSatisfyAllConditions<true>(iterator, iterator, const PayloadValue&, bool*, IdType, IdType,
																		 bool);
template bool SelectIteratorContainer::checkIfSatisfyAllConditions<false>(iterator, iterator, const PayloadValue&, bool*, IdType, IdType,
																		  bool);
...
public:
	MergerRanked(const SelectIterator& ftSelectIter, std::span<const RankT> ftRanks, std::span<const size_t> ftPositions)
		: ftRanks_{ftRanks}, ftPositions_{ftPositions} {
		assertrx_throw(ftSelectIter.size() == 1);
		assertrx_throw(ftSelectIter[0].OwnsFlatIDSet());
		ftIds_ = ftSelectIter[0].TryGetFlatIDSet();
	}
...
class [[nodiscard]] SelectIteratorContainer::MergerRankedImpl {
	struct [[nodiscard]] {
		bool operator()(IdType) const noexcept { return true; }
		bool operator()(faiss::idx_t id) const noexcept { return id >= 0; }
	} static constexpr isValid{};
	struct [[nodiscard]] {
...
public:
	MergerRankedImpl(Merged<desc>& result, IdSetCRef ftIds, std::span<const RankT> ftRanks, std::span<const size_t> ftPositions)
	...
	void operator()(const RerankerLinear& reranker, const concepts::OneOf<HnswKnnRawResult, IvfKnnRawResult> auto& knnRes) const {
		...
		for (size_t i = 0, s = knnIds.size(); i < s && isValid(knnIds[i]); ++i) {
			const IdType id = toRowId(knnIds[i]);
	...
	void operator()(const RerankerRRF& reranker, const concepts::OneOf<IvfKnnRawResult, HnswKnnRawResult> auto& knnRes) const {
		...
		if (!knnIds.empty()) {
			...
			for (size_t i = 0, s = knnIds.size(), knnPos = 1; i < s && isValid(knnIds[i]); ++i) {
...
void SelectIteratorContainer::mergeRanked(RanksHolder::Ptr& ranks, const Reranker& reranker, const NamespaceImpl& ns) {
	...
	IdSetPlain::Ptr resSet = make_intrusive<intrusive_atomic_rc_wrapper<IdSetPlain>>(std::move(mergedIdset));
	ranks = make_intrusive<RanksHolder>();
...
h_vector<size_t, 8> SelectIteratorContainer::CollectDistinctConditions() const {
	h_vector<size_t, 8> res;
	...
	for (const_iterator it = begin; it != end; ++it) {
		...
		it->Visit([&ser](const KnnRawSelectResult&) { ser << "KNN RAW TODO"; },
				  [&](const SelectIteratorsBracket&) {
					  ser << "(\n";
					  dump(level + 1, it.cbegin(), it.cend(), joinItemsProcessors, ser);
					  for (size_t i = 0; i < level; ++i) {
						  ser << "   ";
					  }
					  ser << ')';
				  ...
				  [&ser, &joinItemsProcessors](const JoinSelectIterator& jit) { jit.Dump(ser, joinItemsProcessors); },
