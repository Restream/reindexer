...
void Transaction::Insert(Item&& item) {
	if (impl_) {
		impl_->Insert(std::move(item));
	}
}
void Transaction::Update(Item&& item) {
	if (impl_) {
		impl_->Update(std::move(item));
	}
}
void Transaction::Upsert(Item&& item) {
	if (impl_) {
		impl_->Upsert(std::move(item));
	}
}
void Transaction::Delete(Item&& item) {
	if (impl_) {
		impl_->Delete(std::move(item));
	}
}
void Transaction::Modify(Item&& item, ItemModifyMode mode) {
	if (impl_) {
		impl_->Modify(std::move(item), mode);
	}
}

void Transaction::Modify(Query&& query) {
	if (impl_) {
		impl_->Modify(std::move(query));
	}
}

void Transaction::PutMeta(std::string_view key, std::string_view value) {
	if (impl_) {
		impl_->PutMeta(key, value);
	}
}

void Transaction::MergeTagsMatcher(TagsMatcher&& tm) {
	if (impl_) {
		impl_->MergeTagsMatcher(std::move(tm));
	}
}
