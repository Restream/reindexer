...
public:
	...
	FtCtx(FtCtxType t, RanksHolder::Ptr r) {
		switch (t) {
	...
	void Add(IdType, RankT rank) { data_->ranks->Add(rank); }

	template <typename InputIterator, typename AreaType>
	void Add(InputIterator begin, InputIterator end, RankT rank, AreasInDocument<AreaType>&& areas) {
	...
	template <typename InputIterator, typename AreaType>
	void Add(InputIterator begin, InputIterator end, RankT rank, const std::vector<bool>& mask, AreasInDocument<AreaType>&& areas) {
		intrusive_ptr<FtCtxAreaData<AreaType>> dataArea = static_ctx_pointer_cast<FtCtxAreaData<AreaType>>(data_);
		assertrx_throw(dataArea);
		auto& data = *data_;
		dataArea->area.emplace_back(std::move(areas));
