...
void SearchEngine::AddData(std::string_view src_data, const IdType id, int field, const reindexer::SplitOptions& splitOptions) {
	if (committed_) {
		committed_ = false;
		holder_->Clear();
	}
	searcher_.AddIndex(holder_, src_data, id, field, splitOptions);
}
