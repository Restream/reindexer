...
auto ShardingProxy::isWithSharding(const impl::Query& q, const RdxContext& ctx, int& actualShardId, int64_t& cfgSourceId) const {
...
impl::Query ShardingProxy::NamespaceDataChecker::query() const {
	...
	return *impl::Impl(std::move(query));
}
...
Error ShardingProxy::Update(const impl::Query& query, QueryResults& result, const RdxContext& ctx) {
	try {
		auto updateFn = [this](const impl::Query& q, LocalQueryResults& qr, const RdxContext& ctx) { return impl_.Update(q, qr, ctx); };
...
Error ShardingProxy::Delete(const impl::Query& query, QueryResults& result, const RdxContext& ctx) {
	try {
		auto deleteFn = [this](const impl::Query& q, LocalQueryResults& qr, const RdxContext& ctx) { return impl_.Delete(q, qr, ctx); };
...
Error ShardingProxy::ExecSQL(std::string_view sql, QueryResults& result, unsigned proxyFetchLimit, const RdxContext& ctx) {
	try {
		const impl::Query query = impl::Query::FromSQL(sql);
		switch (query.Type()) {
			case QuerySelect: {
			...
			default:
				return Error(errLogic, "Incorrect sql type {}", int(query.Type()));
		}
...
Error ShardingProxy::Select(const impl::Query& query, QueryResults& result, unsigned proxyFetchLimit, const RdxContext& ctx) {
	try {
		...
		if (auto lckRouterOpt = isWithSharding(query, ctx, actualShardId, shardingVersion)) {
			return executeQueryOnShard(
				*lckRouterOpt, query, result, proxyFetchLimit, ctx,
				[this](const impl::Query& q, LocalQueryResults& qr, const RdxContext& ctx) { return impl_.Select(q, qr, ctx); });
		}
		result.SetShardingConfigVersion(shardingVersion);
		result.AddQr(LocalQueryResults{}, actualShardId);
...
bool ShardingProxy::isSharderQuery(const impl::Query& q, const ShardingRouterLock& shLockShardingRouter) const {
...
Error ShardingProxy::executeQueryOnShard(LockedRouter& lockedShardingRouter, const impl::Query& query, QueryResults& result,
	...
	try {
		...
		if (!isDistributedQuery) {
			...
			impl::Query distributedQuery(query);
			...
			if (distributedQuery.Offset() != QueryEntry::kDefaultOffset) {
				if (distributedQuery.GetSortingEntries().empty()) {
					distributedQuery.CalcTotal(ModeAccurateTotal);
				} else {
...
Error ShardingProxy::executeQueryOnClient(client::Reindexer& connection, const impl::Query& q, client::QueryResults& qrClient,
