...
public:
	...
	Error Update(const impl::Query& q, LocalQueryResults& qr, const RdxContext& ctx) RX_REQUIRES(!mtx_);
	...
	Error Delete(const impl::Query& q, LocalQueryResults& qr, const RdxContext& ctx) RX_REQUIRES(!mtx_);
	Error Select(const impl::Query& q, LocalQueryResults& qr, const RdxContext& ctx) RX_REQUIRES(!mtx_);
...
private:
	...
	using LocalQueryActionFT = Error (ReindexerImpl::*)(const impl::Query&, LocalQueryResults&, const RdxContext&);
	using ProxiedQueryActionFT = Error (client::Reindexer::*)(const impl::Query&, client::QueryResults&) noexcept;
	...
	Error resultFollowerAction(const RdxContext& ctx, LeaderRefT clientToLeader, const impl::Query& query, LocalQueryResults& qr);
	...
	bool shouldProxyQuery(const impl::Query& q);
