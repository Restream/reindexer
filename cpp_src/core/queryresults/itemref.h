...
class [[nodiscard]] ItemRefVector {
	...
	public:
		...
		IteratorImpl& operator=(const IteratorImpl<NotRankedVec::iterator, RankedVec::iterator>& other) noexcept {
			...
			assertf_dbg(this->index() == other.index(), "%d %d", this->index(), other.index());
...
#ifdef REINDEX_CORE_BUILD
		assertf_dbg(b.index() == e.index(), "%d %d", b.index(), e.index());
#endif	// REINDEX_CORE_BUILD
...
#ifdef REINDEX_CORE_BUILD
		assertf_dbg(b.index() == e.index(), "%d %d", b.index(), e.index());
		assertf_dbg(variant_.index() == it.index(), "%d %d", variant_.index(), it.index());
#endif	// REINDEX_CORE_BUILD
...
#ifdef REINDEX_CORE_BUILD
			assertf_dbg(variant_.index() == b.index(), "%d %d", variant_.index(), b.index());
#endif	// REINDEX_CORE_BUILD
