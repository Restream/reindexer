...
struct query_results_ptr : public std::unique_ptr<QueryResultsWrapper, put_results_to_pool> {
	...
	{}
	operator std::unique_ptr<QueryResultsWrapper> () && noexcept {
		return std::unique_ptr<QueryResultsWrapper>{release()};
	}
};

static query_results_ptr new_results() { return res_pool.get(serializedResultsCount.load(std::memory_order_relaxed)); }

static void results2c(std::unique_ptr<QueryResultsWrapper> result, struct reindexer_resbuffer* out, int as_json = 0, int32_t* pt_versions = nullptr,
