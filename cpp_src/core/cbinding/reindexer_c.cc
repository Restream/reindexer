...
reindexer_ret reindexer_select_query(uintptr_t rx, struct reindexer_buffer in, int as_json, int32_t* tm_versions, int tm_versions_count,
	...
	try {
		...
		if (rx) {
			...
			auto q = impl::Query::Deserialize(ser);
			while (!ser.Eof()) {
				const auto joinType = JoinType(ser.GetVarUInt());
				impl::JoinedQuery q1{joinType, impl::Query::Deserialize(ser)};
				if (q1.GetJoinType() == JoinType::Merge) {
					q.Merge(std::move(q1));
				} else {
			...
			if (q.DebugLevel() >= LogError && err.code() != errOK) {
...
reindexer_ret reindexer_delete_query(uintptr_t rx, reindexer_buffer in, reindexer_ctx_info ctx_info) {
	...
	try {
		...
		if (rx) {
			...
			if (q.DebugLevel() >= LogError && res.code() != errOK) {
