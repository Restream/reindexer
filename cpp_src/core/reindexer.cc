...
Error Reindexer::Delete(const impl::Query& q, QueryResults& result) noexcept {
	return callWithConnectCheck([&] {
...
Error Reindexer::Select(const Query& query, QueryResults& result, unsigned proxyFetchLimit) noexcept {
	return impl::Impl{query}.TryExecute([&](const impl::Query& q) noexcept { return Select(q, result, proxyFetchLimit); });
}
Error Reindexer::Select(const impl::Query& q, QueryResults& result, unsigned proxyFetchLimit) noexcept {
	return callWithConnectCheck([&] {
		const auto rdxCtx = impl_->CreateRdxContext(ctx_, [&](WrSerializer& s) { q.GetSQL(s); }, result);
...
Error Reindexer::Update(const impl::Query& q, QueryResults& result) noexcept {
	return callWithConnectCheck([&] {
		const auto rdxCtx = impl_->CreateRdxContext(ctx_, [&](WrSerializer& s) { q.GetSQL(s); }, result);
