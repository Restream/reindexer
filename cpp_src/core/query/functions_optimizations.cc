	...
	if (!optimizedEntries.empty()) {
		...
		for (auto& [i, entry] : optimizedEntries) {
			size_t inserted = std::visit(
				overloaded{[&](QueryEntry& qe) { return queryCopy->template EmplaceQueryEntry<QueryEntry>(i, std::move(qe)); },
						   [&](SubQueryEntry& sqe) { return queryCopy->template EmplaceQueryEntry<SubQueryEntry>(i, std::move(sqe)); }},
				entry);
			if (inserted != 1) {
...
template void OptimizeFunctionEntries(const impl::Query& query, std::optional<impl::Query>& queryCopy, functions::PrecomputedValues&);

template void OptimizeFunctionEntries(const impl::JoinedQuery& query, std::optional<impl::JoinedQuery>& queryCopy,
