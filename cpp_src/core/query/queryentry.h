...
public:
	...
	static constexpr unsigned kDefaultLimit = impl::kQueryMaxLimit;
	static constexpr unsigned kDefaultOffset = impl::kQueryMinOffset;
...
public:
	...
	std::string Dump(const std::vector<impl::Query>& subQueries) const;
...
public:
	...
	void ToDsl(const impl::Query& parentQuery, JsonBuilder& builder) const { return toDsl(cbegin(), cend(), parentQuery, builder); }
	void Serialize(WrSerializer& ser, const std::vector<impl::Query>& subQueries) const { serialize(cbegin(), cend(), ser, subQueries); }
	...
	template <typename JS>
	std::string Dump(const std::vector<JS>& joinItemsProcessors, const std::vector<impl::Query>& subQueries) const {
		WrSerializer ser;
...
private:
	static void toDsl(const_iterator it, const_iterator to, const impl::Query& parentQuery, JsonBuilder&);
	static void serialize(const_iterator it, const_iterator to, WrSerializer&, const std::vector<impl::Query>& subQueries);
...
protected:
	...
	static void dump(size_t level, const_iterator begin, const_iterator end, const std::vector<JS>& joinItemsProcessors,
					 const std::vector<impl::Query>& subQueries, WrSerializer&);
};
