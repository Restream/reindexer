...
using ExpressionValue = std::variant<std::string, VariantArray, functions::FunctionVariant, impl::Query>;
...
public:
	SubQuery(const impl::Query& subQuery) : Expression(ExpressionTypeSubQuery), subQuery_(subQuery) {}
	~SubQuery() override = default;

	void Serialize(WrSerializer& ser) const override;
	const impl::Query& Get() const;
	std::string Dump() const;

private:
	const impl::Query& subQuery_;
};
