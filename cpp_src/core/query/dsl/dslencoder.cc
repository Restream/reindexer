...
static void encodeSingleJoinQuery(const impl::JoinedQuery& joinQuery, JsonBuilder& builder);

static void encodeJoins(const impl::Query& query, JsonBuilder& builder) {
	for (const auto& joinQuery : query.JoinQueries()) {
		if (joinQuery.GetJoinType() == LeftJoin) {
...
static void encodeFilters(const impl::Query& query, JsonBuilder& builder) {
...
static void toDsl(const impl::Query& query, QueryScope scope, JsonBuilder& builder);

static void encodeMergedQueries(const impl::Query& query, JsonBuilder& builder) {
...
static void encodeSelectFunctions(const impl::Query& query, JsonBuilder& builder) {
...
static void encodeAggregationFunctions(const impl::Query& query, JsonBuilder& builder) {
...
static void encodeDropFields(const impl::Query& query, JsonBuilder& builder) {
...
static void encodeUpdateFields(const impl::Query& query, JsonBuilder& builder) {
...
void QueryEntries::toDsl(const_iterator it, const_iterator to, const impl::Query& parentQuery, JsonBuilder& builder) {
	for (; it != to; ++it) {
		auto node = builder.Object();
		...
		it->Visit([](const AlwaysFalse&) { throw Error(errLogic, "Unexpected 'AlwaysFalse' query entry in DSL"); },
				  ...
				  [&node, &parentQuery](const SubQueryEntry& sqe) {
					  node.Put("cond"sv, dsl::get(dsl::kCondMap, CondType(sqe.Condition())));
					  {
						  ...
						  dsl::toDsl(parentQuery.SubQueries()[sqe.QueryIndex()], dsl::QueryScope::Subquery, subquery);
					  }
				  ...
				  [&node, &parentQuery](const JoinQueryEntry& jqe) {
					  assertrx(jqe.joinIndex < parentQuery.JoinQueries().size());
					  dsl::encodeSingleJoinQuery(parentQuery.JoinQueries()[jqe.joinIndex], node);
				  },
