...
public:
	...
	Error ExecSelect(const impl::Query& query, QueryResults& result, const sharding::ConnectionsVector& connections, const RdxContext& ctx,
					 std::function<Error(const impl::Query&, LocalQueryResults&, const RdxContext&)>&& localAction);
