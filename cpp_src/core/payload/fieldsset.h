...
public:
	...
	bool contains(const TagsPath& tagsPath) const noexcept {
		for (const FieldsPath& path : tagsPaths_) {
			if (auto* tp = std::get_if<TagsPath>(&path); tp) {
				if (*tp == tagsPath) {
					return true;
				}
			} else if (auto* itp = std::get_if<IndexedTagsPath>(&path); itp) {
				if (itp->Compare(tagsPath)) {
					return true;
				}
			} else {
				assertrx_dbg(false);
	...
	bool contains(const IndexedTagsPath& tagsPath) const noexcept {
		for (const FieldsPath& path : tagsPaths_) {
			if (auto* tp = std::get_if<TagsPath>(&path); tp) {
				if (tagsPath.Compare(*tp)) {
					return true;
				}
			} else if (auto* itp = std::get_if<IndexedTagsPath>(&path); itp) {
				if (Compare<IgnoreAllOmittedIndexes>(*itp, tagsPath)) {
					return true;
				}
			} else {
				assertrx_dbg(false);
	...
	bool contains(const FieldsPath& fieldsPath) const noexcept {
		if (auto* tp = std::get_if<TagsPath>(&fieldsPath); tp) {
			return contains(*tp);
		} else if (auto* itp = std::get_if<IndexedTagsPath>(&fieldsPath); itp) {
			return contains(*itp);
		}
		assertrx_dbg(false);
		return false;
	...
	bool match(const TagsPath& tagsPath) const noexcept {
		...
		for (auto& path : tagsPaths_) {
			if (auto* tp = std::get_if<TagsPath>(&path); tp) {
				if (comparePaths(tagsPath, *tp)) {
					return true;
				}
			} else if (auto* itp = std::get_if<IndexedTagsPath>(&path); itp) {
				if (comparePaths(tagsPath, *itp)) {
					return true;
				}
			} else {
				assertrx_dbg(false);
	...
	bool isTagsPathIndexed(size_t idx) const noexcept {
		...
		if (auto* tp = std::get_if<TagsPath>(&tagsPaths_[idx]); tp) {
			return false;
		} else if (auto* itp = std::get_if<IndexedTagsPath>(&tagsPaths_[idx]); itp) {
			return true;
		}
		assertrx_dbg(false);
		return false;
