...
public:
	...
	bool Drop(std::string_view field) { return clone()->Drop(field); }
	int FieldByName(std::string_view field) const { return get()->FieldByName(field); }
	bool FieldByName(std::string_view name, int &field) const { return get()->FieldByName(name, field); }
	bool Contains(std::string_view field) const { return get()->Contains(field); }
	int FieldByJsonPath(std::string_view jsonPath) const { return get()->FieldByJsonPath(jsonPath); }
