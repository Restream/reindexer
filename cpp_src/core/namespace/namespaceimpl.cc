		...
		for (auto&& step : tx.GetSteps()) {
			...
			switch (step.type_) {
				...
				case TransactionStep::Type::Query: {
					...
					std::optional<impl::Query> query{std::in_place, *impl::Impl{std::move(*data.query)}};
...
void NamespaceImpl::doUpdateTr(LocalQueryResults& result, UpdatesContainer& pendedRepl, const impl::Query& query, const NsContext& ctx,
...
void NamespaceImpl::doUpdate(LocalQueryResults& result, UpdatesContainer& pendedRepl, const impl::Query& query, const NsContext& ctx,
							 const functions::PrecomputedValues& precomputedValues) {
	if (!pkFields()) {
		throwCannotModifyNsWithoutPK();
	}
...
void NamespaceImpl::doDeleteTr(LocalQueryResults& result, UpdatesContainer& pendedRepl, const impl::Query& query, const NsContext& ctx,
							   const functions::PrecomputedValues& precomputedValues) {
	NsSelecter selecter(this);
...
void NamespaceImpl::doDelete(LocalQueryResults& result, UpdatesContainer& pendedRepl, const impl::Query& query, const NsContext& ctx,
...
void NamespaceImpl::removeExpiredItems(RdxActivityContext* ctx) {
	...
	for (const std::unique_ptr<Index>& index : indexes_) {
		...
		const impl::Query q = *impl::Impl{Query(name_).Where(index->Name(), CondLt, expirationThreshold)};
...
void NamespaceImpl::getFromJoinCache(const impl::Query& q, const impl::JoinedQuery& jq, joins::CacheRes& out) const {
...
void NamespaceImpl::getFromJoinCache(const impl::Query& q, joins::CacheRes& out) const {
	if (config_.cacheMode == CacheModeOff || !indexOptimizer_.IsOptimizationCompleted()) {
		return;
	}
	out.key.SetData(q);
