...
void NamespaceImpl::checkSnapshotLSN(lsn_t lsn) {
	...
	if rx_unlikely (!kDisableSnapshotCheck && wal_.LastLSN().Counter() > lsn.Counter()) {
		// FIXME: Has to be fixed or disabled before v5.5.0
		// Do not expect to get this error in tests scenarious
		std::cerr << fmt::format(
						 "*REPORT THIS: related issue #2143*\nTarget namespace '{}' has unexpected LSN counter: {}. First LSN in snapshot "
						 "chunk is {}. Snapshot's data are incompatible",
						 name_, wal_.LastLSN(), lsn)
				  << std::endl;
