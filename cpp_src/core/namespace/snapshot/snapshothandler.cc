...
Snapshot SnapshotHandler::CreateSnapshot(const SnapshotOpts& opts) const {
	...
	try {
		...
		impl::Query q = *impl::Impl{Query(ns_.name_).Where("#lsn", CondGt, int64_t(from.LSN())).SelectAllFields()};
		...
		{
			impl::Query q = *impl::Impl{Query(ns_.name_).Where("#lsn", CondGe, int64_t(minLsn)).SelectAllFields()};
			SelectCtx selCtx(q, nullptr, &walQr.GetFloatVectorsHolder());
			FtFunctionsHolder func;
			selCtx.functions = &func;
			selCtx.contextCollectingMode = true;
		...
		{
			impl::Query q = *impl::Impl{Query(ns_.name_).Where("#lsn", CondAny, VariantArray{}).SelectAllFields()};
...
void SnapshotHandler::applyRealRecord(lsn_t lsn, const SnapshotRecord& snRec, const ChunkContext& chCtx, UpdatesContainer& pendedRepl) {
	...
	switch (rec.type) {
		...
		case WalUpdateQuery: {
			...
			const auto q = impl::Query::FromSQL(rec.data);
			switch (q.Type()) {
				case QueryDelete:
