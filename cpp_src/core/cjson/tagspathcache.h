...
class TagsPathCache {
public:
	void set(const int *tagsPath, size_t len, int field) {
		assert(len);
		auto cache = this;
		for (;;) {
			int tag = *tagsPath++;
	...
	int lookup(const int *tagsPath, size_t len) const {
		assert(len);
		auto cache = this;
		for (;;) {
			int tag = *tagsPath++;
	...
	void walk(int *path, int depth, std::function<void(int, int)> visitor) const {
		int &i = path[depth];
