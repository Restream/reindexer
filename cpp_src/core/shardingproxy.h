...
public:
	...
	Error Update(const impl::Query& query, QueryResults& result, const RdxContext& ctx);
	...
	Error Delete(const impl::Query& query, QueryResults& result, const RdxContext& ctx);
	Error ExecSQL(std::string_view sql, QueryResults& result, unsigned proxyFetchLimit, const RdxContext& ctx);
	Error Select(const impl::Query& query, QueryResults& result, unsigned proxyFetchLimit, const RdxContext& ctx);
...
private:
	...
	auto isWithSharding(const impl::Query& q, const RdxContext& ctx, int& actualShardId, int64_t& cfgSourceId) const;
	...
	bool isSharderQuery(const impl::Query& q, const ShardingRouterLock& shLockShardingRouter) const;
	...
	Error executeQueryOnShard(LockedRouter&, const impl::Query& query, QueryResults& result, unsigned proxyFetchLimit, const RdxContext&,
							  LocalFT&&) noexcept;
	template <typename CalucalteFT>
	Error executeQueryOnClient(client::Reindexer& connection, const impl::Query& q, client::QueryResults& qrClient,
