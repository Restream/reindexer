...
Error Transaction::Modify(impl::Query&& query, lsn_t lsn) { return modify(std::move(query), InternalRdxContext(std::move(lsn))); }
...
Error Transaction::modify(impl::Query&& query, InternalRdxContext&& ctx) {
	if (!IsFree()) {
