...
Error ReindexerImpl::Update(const impl::Query& query, QueryResults& result, const InternalRdxContext& ctx) {
...
Error ReindexerImpl::Delete(const impl::Query& query, QueryResults& result, const InternalRdxContext& ctx) {
	Error err = result.setClient(shared_from_this());
	if (!err.ok()) {
		return err;
	}
...
Error ReindexerImpl::Select(const impl::Query& query, QueryResults& result, const InternalRdxContext& ctx) {
	Error err = result.setClient(shared_from_this());
	if (!err.ok()) {
		return err;
	}
...
Error ReindexerImpl::modifyTx(Transaction& tr, impl::Query&& q, const InternalRdxContext& ctx) {
...
void ReindexerImpl::coroInterpreter(Connection<DatabaseCommand>& conn, ConnectionsPool<DatabaseCommand>& pool, uint32_t tid) noexcept {
	...
	for (std::pair<DatabaseCommand, bool> v = conn.PopCmd(); v.second == true; v = conn.PopCmd()) {
		...
		switch (cmd->id) {
			...
			case DbCmdUpdateQ: {
				execCommand(cmd, [&conn, &cmd](const impl::Query& query, CoroQueryResults& result) {
					return conn.rx.Update(query, result, cmd->ctx);
				});
				break;
			}
			case DbCmdDelete: {
			...
			case DbCmdDeleteQ: {
				execCommand(cmd, [&conn, &cmd](const impl::Query& query, CoroQueryResults& result) {
					return conn.rx.Delete(query, result, cmd->ctx);
				});
				break;
			}
			case DbCmdNewItem: {
			...
			case DbCmdSelect: {
				execCommand(cmd, [&conn, &cmd](const impl::Query& query, CoroQueryResults& result) {
					return conn.rx.Select(query, result, cmd->ctx);
				});
				break;
			}
			case DbCmdGetMeta: {
			...
			case DbCmdModifyTx: {
				auto cd = dynamic_cast<DatabaseCommandData<Error, CoroTransaction&, impl::Query>*>(cmd);
				assertrx(cd);
				...
				if (txConn) {
					...
					switch (std::get<1>(cd->arguments).Type()) {
						case QueryUpdate:
							err = txConn
						...
						case QuerySelect:
						case QueryTruncate:
							err = Error(errParams, "Incorrect query type in transaction modify {}", int(std::get<1>(cd->arguments).Type()));
					}
				}
				if (cd->ctx.cmpl()) {
					cd->ctx.cmpl()(err);
