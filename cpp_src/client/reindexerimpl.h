...
public:
	...
	Error Update(const impl::Query& query, QueryResults& result, const InternalRdxContext& ctx);
	...
	Error Delete(const impl::Query& query, QueryResults& result, const InternalRdxContext& ctx);
	Error ExecSQL(std::string_view query, QueryResults& result, const InternalRdxContext& ctx);
	Error Select(const impl::Query& query, QueryResults& result, const InternalRdxContext& ctx);
	...
	Error GetSqlSuggestions(std::string_view sqlQuery, int pos, SQLSuggestions& suggestions);
...
private:
	...
	Error modifyTx(Transaction& tr, impl::Query&& q, const InternalRdxContext& ctx);
