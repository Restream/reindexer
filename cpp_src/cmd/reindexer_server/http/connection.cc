...
void Connection::callback(ev::io & /*watcher*/, int revents) {
	...
	if (curEvents_ != nevents && fd_ >= 0) {
		(curEvents_) ? io_.set(nevents) : io_.start(fd_, ev::READ);
		curEvents_ = nevents;
	}
}
...
void Connection::parseRequest() {
	...
	while (rdBuf_.size()) {
		if (!bodyLeft_) {
			...
			((char *)request_.method)[method_len] = 0;
			((char *)request_.path)[path_len] = 0;
			request_.headers.clear();
			request_.params.clear();

			char *p = strchr((char *)request_.path, '?');
			if (p) {
				*p = 0;
				parseParams(p + 1);
			}
			...
			for (int i = 0; i < (int)num_headers; i++) {
				...
				((char *)hdr.name)[headers[i].name_len] = 0;
				((char *)hdr.val)[headers[i].value_len] = 0;
				for (char *p = (char *)hdr.name; *p; ++p) *p = tolower(*p);
				for (char *p = (char *)hdr.val; *p; ++p) *p = tolower(*p);
...
ssize_t Connection::ResponseWriter::Write(const void *buf, size_t size) {
	...
	conn_->wrBuf_.write((const char *)buf, size);
	if (isChunkedResponse()) {
		conn_->wrBuf_.write(kStrEOL, sizeof(kStrEOL) - 1);
	}
...
ssize_t Connection::BodyReader::Read(void *buf, size_t size) {
	size_t readed = conn_->rdBuf_.read((char *)buf, std::min((ssize_t)size, conn_->bodyLeft_));
