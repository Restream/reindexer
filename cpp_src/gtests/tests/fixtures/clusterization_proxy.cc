...
void ClusterizationProxyApi::ItemTracker::AddCommited(std::string&& json, ItemInfo&& info) {
...
void ClusterizationProxyApi::ItemTracker::AddTxData(std::unordered_map<std::string, ItemInfo>& into,
...
void ClusterizationProxyApi::ItemTracker::AddError(std::string&& json, ItemInfo&& info) {
	std::lock_guard lck(mtx_);
...
void ClusterizationProxyApi::ItemTracker::AddUnknown(std::string&& json, ItemInfo&& info) {
	std::lock_guard lck(mtx_);
...
void ClusterizationProxyApi::ItemTracker::Validate(client::QueryResults& qr) {
...
const ClusterizationApi::Defaults& ClusterizationProxyApi::GetDefaults() const {
	static Defaults defs{14100, 16100, fs::JoinPath(fs::GetTempDir(), "rx_test/ClusterizationProxyApi")};
	return defs;
}

int ClusterizationProxyApi::GetRandFollower(int clusterSize, int leaderId) {
