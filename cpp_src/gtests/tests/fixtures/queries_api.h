...
public:
	...
	reindexer::Query& Distinct(Str&& name) & {
		if (!reindexer::strEmpty(name)) {
			reindexer::Query::Distinct({std::string(name)});
		}
		return *this;
	}
	template <reindexer::concepts::ConvertibleToString Str>
	...
	const QueryImpl& Impl() const& { return *reindexer::impl::Impl<const reindexer::Query&>{*this}; }
	QueryImpl& Impl() & { return *reindexer::impl::Impl<reindexer::Query&>{*this}; }
};
...
public:
	...
	void ExecuteAndVerify(Q&& query, Args&&... args) {
		...
		if constexpr (std::is_same_v<std::remove_cvref_t<Q>, QueryImpl>) {
			Verify(qr, std::forward<Q>(query), *rt.reindexer);
		} else if constexpr (std::is_same_v<std::remove_cvref_t<Q>, TestQuery>) {
			Verify(qr, forward_like<Q>(query.Impl()), *rt.reindexer);
		} else {
			Verify(qr, *Impl{std::forward<Q>(query)}, *rt.reindexer);
		}
	...
	void ExecuteAndVerifyWithSql(Q&& q) {
		if constexpr (std::is_same_v<std::remove_cvref_t<Q>, QueryImpl>) {
			...
			ExecuteAndVerifyWithSql(q.Impl());
		} else {
			ExecuteAndVerifyWithSql(*Impl{q});
		}
	}

	void ExecuteAndVerifyWithSql(QueryImpl& query) {
		ExecuteAndVerify(query);
		auto queryFromSql = QueryImpl::FromSQL(query.GetSQL());
		queryFromSql.Strict(query.GetStrictMode());
		queryFromSql.DebugLevel(query.DebugLevel());
	...
	void executeAndVerify(QueryImpl&& query, QueryResults& qr, Args&&... args) {
		query.Explain();
		rt.Select(query, qr);
	...
	template <typename... Args>
	void executeAndVerify(QueryImpl& query, QueryResults& qr, Args&&... args) {
		executeAndVerify(QueryImpl{query}, qr, std::forward<Args>(args)...);
	}

	template <typename Q, typename... Args>
	void ExecuteAndVerify(Q&& q, QueryResults& qr, Args&&... args) {
		if constexpr (std::is_same_v<std::remove_cvref_t<Q>, QueryImpl>) {
			executeAndVerify(q, qr, std::forward<Args>(args)...);
		} else if constexpr (std::is_same_v<std::remove_cvref_t<Q>, TestQuery>) {
			executeAndVerify(q.Impl(), qr, std::forward<Args>(args)...);
		} else {
			executeAndVerify(*Impl{q}, qr, std::forward<Args>(args)...);
		}
	}
	...
	void ExecuteAndVerifyWithSql(Q&& q, QueryResults& qr) {
		if constexpr (std::is_same_v<std::remove_cvref_t<Q>, QueryImpl>) {
			...
			ExecuteAndVerifyWithSql(*Impl{q}, qr);
		}
	}

	void ExecuteAndVerifyWithSql(QueryImpl& query, QueryResults& qr) {
		ExecuteAndVerify(query, qr);
		Query queryFromSql = Query::FromSQL(query.GetSQL()).Strict(query.GetStrictMode()).Debug(query.DebugLevel());
