...
enum class FieldType { Bool, Int, Int64, Double, String, Point, Struct };
...
public:
	...
	fuzzing::FieldType RndPkIndexFieldType() {
		return static_cast<fuzzing::FieldType>(
			RndInt(static_cast<int>(fuzzing::FieldType::Int), static_cast<int>(fuzzing::FieldType::String)));
	}
	std::string IndexFieldType(fuzzing::FieldType);
	fuzzing::FieldType RndFieldType(fuzzing::FieldType type) {
		if (RndErr()) {
			return RndWhich<fuzzing::FieldType, 1, 1, 1, 1, 1, 1, 1>();
		}
		return type;
	}
	...
	size_t ArraySize() {
		...
		enum Size : uint8_t { Short, Normal, Long, VeryLong };
	...
	size_t IndexesCount() {
		...
		enum Count : uint8_t { Few, Normal, Many, TooMany };
	...
	size_t CompositeIndexSize() {
		if (RndErr()) {
			enum Err : uint8_t { Zero, One, TooMany };
	...
	size_t ItemsCount() {
		enum Count : uint8_t { Short, Normal, Long, VeryLong };
