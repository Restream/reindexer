...
IsArray NsScheme::IsArray(const FieldPath& path) const noexcept {
	...
	for (size_t i = 0, s = path.size() - 1; i < s; ++i) {
		...
		if (idx.array == IsArray::Yes) return IsArray::Yes;
		std::visit(
...
void NsScheme::AddIndex(const FieldPath& path, size_t index, IsSparse isSparse) {
	assertrx(!path.empty());
	if (isSparse == IsSparse::No) {
		ns_.sparse = IsSparse::No;
	}
	Node::Children* ptr = &std::get<Node::Children>(ns_.content);
	for (size_t i = 0, s = path.size() - 1; i < s; ++i) {
		assertrx(ptr->size() > path[i]);
		if (isSparse == IsSparse::No) {
			(*ptr)[path[i]].sparse = IsSparse::No;
		}
...
FieldPath NsScheme::AddRndPkField(RandomGenerator& rnd) {
	...
	children.back().array = IsArray::No;
	children.back().sparse = IsSparse::No;
	return {children.size() - 1};
}

void NsScheme::addIndex(Node& node, size_t index, IsSparse isSparse) {
	if (isSparse == IsSparse::No) {
		node.sparse = IsSparse::No;
	}
...
void NsScheme::toJson(reindexer::JsonBuilder& builder, const Node::Children& children, RandomGenerator& rnd,
	...
	for (const Node& n : children) {
		...
		if (rnd.RndArrayField(n.array) == IsArray::Yes) {
