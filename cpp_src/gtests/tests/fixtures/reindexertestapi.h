...
public:
	...
	void Update(const QueryImpl& q, QueryResultsType& qr);
	Error UpdateErr(const Query& q, QueryResultsType& qr);
	Error UpdateErr(const QueryImpl& q, QueryResultsType& qr);
	size_t Update(const QueryImpl& q);
	size_t Update(const Query& q);
	QueryResultsType UpdateQR(const QueryImpl& q);
	QueryResultsType UpdateQR(const Query& q);
	void Select(const QueryImpl& q, QueryResultsType& qr) const;
	void Select(const Query& q, QueryResultsType& qr) const;
	Error SelectErr(const QueryImpl& q, QueryResultsType& qr) const;
	Error SelectErr(const Query& q, QueryResultsType& qr) const;
	Error SelectWithTimeout(const QueryImpl& q, QueryResultsType& qr, std::chrono::milliseconds timeout) const;
	Error SelectWithTimeout(const Query& q, QueryResultsType& qr, std::chrono::milliseconds timeout) const;
	QueryResultsType Select(const QueryImpl& q) const;
	...
	size_t Delete(const QueryImpl& q);
	size_t Delete(const Query& q);
	void Delete(const QueryImpl& q, QueryResultsType& qr);
