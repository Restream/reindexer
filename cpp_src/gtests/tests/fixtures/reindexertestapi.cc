...
template <typename DB>
Error ReindexerTestApi<DB>::UpdateErr(const QueryImpl& q, QueryResultsType& qr) {
	return reindexer->WithTimeout(kBasicTimeout).Update(q, qr);
}

template <typename DB>
void ReindexerTestApi<DB>::Update(const QueryImpl& q, QueryResultsType& qr) {
...
template <typename DB>
size_t ReindexerTestApi<DB>::Update(const QueryImpl& q) {
	QueryResultsType qr;
	Update(q, qr);
	return qr.Count();
}
...
size_t ReindexerTestApi<DB>::Update(const Query& q) {
	return Update(*Impl{q});
}

template <typename DB>
typename ReindexerTestApi<DB>::QueryResultsType ReindexerTestApi<DB>::UpdateQR(const QueryImpl& q) {
	QueryResultsType qr;
	Update(q, qr);
	return qr;
}
...
typename ReindexerTestApi<DB>::QueryResultsType ReindexerTestApi<DB>::UpdateQR(const Query& q) {
	return UpdateQR(*Impl{q});
}

template <typename DB>
void ReindexerTestApi<DB>::Select(const QueryImpl& q, QueryResultsType& qr) const {
...
template <typename DB>
Error ReindexerTestApi<DB>::SelectErr(const QueryImpl& q, QueryResultsType& qr) const {
	return SelectWithTimeout(q, qr, kBasicTimeout);
}

template <typename DB>
Error ReindexerTestApi<DB>::SelectErr(const Query& q, QueryResultsType& qr) const {
	return SelectErr(*Impl{q}, qr);
}

template <typename DB>
Error ReindexerTestApi<DB>::SelectWithTimeout(const QueryImpl& q, QueryResultsType& qr, std::chrono::milliseconds timeout) const {
...
Error ReindexerTestApi<DB>::SelectWithTimeout(const Query& q, QueryResultsType& qr, std::chrono::milliseconds timeout) const {
	return SelectWithTimeout(*Impl{q}, qr, timeout);
}

template <typename DB>
typename ReindexerTestApi<DB>::QueryResultsType ReindexerTestApi<DB>::Select(const QueryImpl& q) const {
	QueryResultsType qr;
	Select(q, qr);
	return qr;
}
...
template <typename DB>
size_t ReindexerTestApi<DB>::Delete(const QueryImpl& q) {
	QueryResultsType qr;
...
size_t ReindexerTestApi<DB>::Delete(const Query& q) {
	return Delete(*Impl{q});
}

template <typename DB>
void ReindexerTestApi<DB>::Delete(const QueryImpl& q, QueryResultsType& qr) {
	auto err = reindexer->WithTimeout(kBasicTimeout).Delete(q, qr);
	EXPECT_TRUE(err.ok()) << err.what() << "; " << q.GetSQL(QueryDelete);
}
