...
TEST_F(NsApi, QueryperfstatsNsDummyTest) {
	...
	auto testQuery = QueryImpl(default_namespace, 0, 0, ModeAccurateTotal);
...
static void setAndCheckArrayItem(const std::shared_ptr<reindexer::Reindexer>& reindexer, std::string_view ns,
								 const std::string& fullItemPath, std::string_view jsonPath, std::string_view description,
...
static void checkQueryDsl(const QueryImpl& src) {
	const std::string dsl = src.GetJSON();
	auto dst = QueryImpl::CreateEmpty();
	EXPECT_NO_THROW(dst = QueryImpl::FromJSON(dsl));
	...
	if (objectValues) {
		...
		EXPECT_EQ(src.Aggregations(), dst.Aggregations());
		...
		EXPECT_EQ(src.SelectFunctions(), dst.SelectFunctions());
		EXPECT_EQ(src.JoinQueries(), dst.JoinQueries());
		EXPECT_EQ(src.MergeQueries(), dst.MergeQueries());
	} else {
		EXPECT_EQ(dst, src);
	}
}

static void checkQueryDsl(const Query& src) { checkQueryDsl(*reindexer::impl::Impl{src}); }
...
TEST_F(NsApi, TestModifyQueriesSqlEncoder) {
...

	{
		...
		q.Merge(QueryImpl("ns3"));
		q.Merge(QueryImpl("ns4"));
		QueryImpl dst = QueryImpl::CreateEmpty();
		EXPECT_NO_THROW(dst = QueryImpl::FromJSON(q.GetJSON()));
...
TEST_F(NsApi, MultiDimensionalArrayQueryErrors) {
	...
	auto testSet = [this](const std::string& field) {
