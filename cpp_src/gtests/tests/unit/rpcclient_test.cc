		...
		loop.spawn([&, index = i] {
			...
			for (size_t j = 0; j < kQueriesCount; ++j) {
				loop.spawn([&] {
					...
					err = rx.Select(QueryImpl(kNamespaceName), qr);
					EXPECT_EQ(err.code(), errTimeout);
				});
			}
			wg.wait();
	...
	loop.spawn(exceptionWrapper([&loop] {
		...
		auto selectFn = [&loop, &rx, &nsName](size_t cnt) {
			...
			for (size_t j = 0; j < kMultiplier * cnt; ++j) {
				if (j % kMultiplier == 0) {
					reindexer::client::CoroQueryResults qr;
					auto err = rx.Select(QueryImpl(nsName), qr);
					ASSERT_TRUE(err.ok()) << err.what();
					for (auto& it : qr) {
		...
		reindexer::client::CoroQueryResults qr;
		err = rx.Select(QueryImpl(nsName), qr);
		ASSERT_TRUE(err.ok()) << err.what();
...
TEST_F(RPCClientTestApi, UnknownResultsFlag) {
	...
	loop.spawn([&loop, &finished] {
		...
		err = rx.Select(*Impl{Query(reindexer::kConfigNamespace).Where("type", CondEq, "namespaces")}, qr);
		ASSERT_TRUE(err.ok()) << err.what();
...
TEST_F(RPCClientTestApi, FetchingWithJoin) {
	...
	loop.spawn(exceptionWrapper([&loop] {
		...
		err = rx.Select(*Impl{Query(kLeftNsName).Join(InnerJoin, Query(kRightNsName)).On("id", CondEq, "id").Sort("id", false)}, qr);
		ASSERT_TRUE(err.ok()) << err.what();
...
TEST_F(RPCClientTestApi, QRWithMultipleIterationLoops) {
	...
	loop.spawn(exceptionWrapper([&loop, this] {
		...
		client::CoroQueryResults qr;
		err = rx.Select(QueryImpl(kNsName), qr);
		ASSERT_TRUE(err.ok()) << err.what();
		ASSERT_EQ(qr.Count(), kNsSize);
		WrSerializer ser;
...
TEST_F(RPCClientTestApi, AggregationsFetching) {
	...
	loop.spawn(exceptionWrapper([&loop, this, kItemsCount] {
		...
		{
			reindexer::client::CoroQueryResults qr;
			const auto q = *Impl{Query(nsName).Distinct({"id"}).ReqTotal().Explain()};
			err = rx.Select(q, qr);
			ASSERT_TRUE(err.ok()) << err.what();
...
TEST_F(RPCClientTestApi, SubQuery) {
	...
	loop.spawn(exceptionWrapper([&loop, this] {
		...
		{
			client::CoroQueryResults qr;
			err = rx.Select(*Impl{Query(kLeftNsName).Where("id", CondSet, Query(kRightNsName).Select("id").Where("id", CondLt, kHalfSize))},
							qr);
			ASSERT_TRUE(err.ok()) << err.what();
		...
		}
		{
			const int limit = 10;
			client::CoroQueryResults qr;
			err = rx.Select(*Impl{Query(kLeftNsName)
									  .Where(Query(kRightNsName).Where("id", CondLt, kHalfSize).ReqTotal(), CondEq, {kHalfSize})
									  .Limit(limit)},
							qr);
			ASSERT_TRUE(err.ok()) << err.what();
...
TEST_F(RPCClientTestApi, QuerySelectDWithin) {
	...
	loop.spawn(exceptionWrapper([&loop, this] {
		...
		{
			client::CoroQueryResults qr;
			err = rx.Select(*Impl{Query(kNsName).DWithin("point", Point(3, 2.5), 2.2)}, qr);
			ASSERT_TRUE(err.ok()) << err.what();
...
TEST_F(RPCClientTestApi, QuerySetObjectUpdate) {
	...
	loop.spawn(exceptionWrapper([&loop, this] {
		...
		{
			err = rx.Update(
				*Impl{Query(kNsName).Where("id", CondGe, "0").SetObject("nested", Variant(std::string(R"([{"field": 1240}])")))}, qr);
			ASSERT_FALSE(err.ok());
