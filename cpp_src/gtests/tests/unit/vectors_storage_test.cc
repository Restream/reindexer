...
protected:
	...
	JsonResults requestAllData() {
		const static auto kRequestAllQ = Query(kNsName<isArray>).Sort(std::string(kFieldNameId), false).SelectAllFields();
	...
	std::vector<Query> prepareANNQueries(size_t count, std::string_view idx, size_t dims, const KnnSearchParams& param) {
		...
		while (ret.size() != count) {
			...
			auto query = Query{kNsName<isArray>}.WhereKNN(std::string(idx), FloatVector{buf}, param).WithRank().SelectAllFields();
