...
TEST_F(ReplicationLoadApi, WALResizeStaticData) {
	...
	ASSERT_NO_FATAL_FAILURE(SetWALSize(masterId_, 1000, nsName));
	{
		...
		master.Select(Query(nsName).Where("#lsn", CondGt, int64_t(0)), qr);
		EXPECT_EQ(qr.Count(), 4);
	}
	{
		BaseApi::QueryResultsType qr(kResultsWithPayloadTypes | kResultsCJson | kResultsWithItemID | kResultsWithRaw);
		master.Select(Query(nsName).Where("#lsn", CondGt, int64_t(2)), qr);
	...
	{
		BaseApi::QueryResultsType qr(kResultsWithPayloadTypes | kResultsCJson | kResultsWithItemID | kResultsWithRaw);
		master.Select(Query(nsName).Where("#lsn", CondGt, int64_t(503)), qr);
		EXPECT_EQ(qr.Count(), 1);
	}
	{
		BaseApi::QueryResultsType qr(kResultsWithPayloadTypes | kResultsCJson | kResultsWithItemID | kResultsWithRaw);
		Error err = master.SelectErr(Query(nsName).Where("#lsn", CondGt, int64_t(504)), qr);
		EXPECT_EQ(err.code(), errOutdatedWAL) << err.what();
		EXPECT_EQ(qr.Count(), 0);
	}
	{
		Error err = master.SelectErr(Query(nsName).Where("#lsn", CondGt, int64_t(404)), qrLast100_1);
	...
	{
		BaseApi::QueryResultsType qr(kResultsWithPayloadTypes | kResultsCJson | kResultsWithItemID | kResultsWithRaw);
		Error err = master.SelectErr(Query(nsName).Where("#lsn", CondGt, int64_t(403)), qr);
		EXPECT_EQ(err.code(), errOutdatedWAL) << err.what();
	}
	{
		master.Select(Query(nsName).Where("#lsn", CondGt, int64_t(404)), qrLast100_2);
	...
	{
		master.Select(Query(nsName).Where("#lsn", CondGt, int64_t(404)), qrLast100_3);
...
TEST_F(ReplicationLoadApi, WALResizeDynamicData) {
	...
	{
		BaseApi::QueryResultsType qr(kResultsWithPayloadTypes | kResultsCJson | kResultsWithItemID | kResultsWithRaw);
		Error err = master.SelectErr(Query(nsName).Where("#lsn", CondGt, int64_t(453)), qr);
		EXPECT_EQ(err.code(), errOutdatedWAL) << err.what();
	}
	{
		BaseApi::QueryResultsType qr(kResultsWithPayloadTypes | kResultsCJson | kResultsWithItemID | kResultsWithRaw);
		master.Select(Query(nsName).Where("#lsn", CondGt, int64_t(454)), qr);
	...
	{
		BaseApi::QueryResultsType qr(kResultsWithPayloadTypes | kResultsCJson | kResultsWithItemID | kResultsWithRaw);
		Error err = master.SelectErr(Query(nsName).Where("#lsn", CondGt, int64_t(853)), qr);
		EXPECT_EQ(err.code(), errOutdatedWAL) << err.what();
		EXPECT_EQ(qr.Count(), 0);
	}
	{
		BaseApi::QueryResultsType qr(kResultsWithPayloadTypes | kResultsCJson | kResultsWithItemID | kResultsWithRaw);
		master.Select(Query(nsName).Where("#lsn", CondGt, int64_t(854)), qr);
		EXPECT_EQ(qr.Count(), 200);
	}
	{
		BaseApi::QueryResultsType qr(kResultsWithPayloadTypes | kResultsCJson | kResultsWithItemID | kResultsWithRaw);
		master.Select(Query(nsName).Where("#lsn", CondGt, int64_t(1053)), qr);
		EXPECT_EQ(qr.Count(), 1);
	}
	{
		BaseApi::QueryResultsType qr(kResultsWithPayloadTypes | kResultsCJson | kResultsWithItemID | kResultsWithRaw);
		Error err = master.SelectErr(Query(nsName).Where("#lsn", CondGt, int64_t(1054)), qr);
		EXPECT_EQ(err.code(), errOutdatedWAL) << err.what();
		EXPECT_EQ(qr.Count(), 0);
	}
}
