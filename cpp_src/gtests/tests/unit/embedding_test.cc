...
TEST_F(EmbeddingTest, SqlQueryWhereKNN) try {
	...
	struct {
		...
		{Query("ns"sv).WhereKNN("hnsw", k1st, reindexer::HnswSearchParams{}.K(4'291).Radius(1.043).Ef(100'000)).SelectAllFields(),
		 "SELECT *, vectors() FROM ns WHERE KNN(hnsw, \'" + k1st + "\', k=4291, radius=1.043, ef=100000)"},
		{Query("ns"sv).WhereKNN("bf", k2nd, reindexer::BruteForceSearchParams{}.K(8'184)).Select("vectors()"),
		 "SELECT vectors() FROM ns WHERE KNN(bf, \'" + k2nd + "\', k=8184)"},
		{Query("ns"sv).WhereKNN("ivf", k3rd, reindexer::IvfSearchParams{}.K(823).Radius(1.32).NProbe(5)).Select("hnsw").Select("vectors()"),
