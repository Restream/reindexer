...
void FloatVector::checkEmptyIndexSelection(std::string_view ns, const std::string& vectorIndex) {
...
void FloatVector::runMultithreadQueries(size_t threads, size_t queriesPerThread, std::string_view nsName, const std::string& fieldName,
...
static void checkIndexMemstat(ReindexerTestApi<reindexer::Reindexer>& rx, std::string_view ns, std::string_view vectorIndex, size_t dims,
	...
	auto dataSize = dims * sizeof(float);
	if (M) {
		if (*M != 0) {
			// HNSW size per element, not required for Bruteforce
			dataSize += 2 * (*M) * sizeof(hnswlib::tableint) + sizeof(hnswlib::linklistsizeint) + sizeof(uint64_t);
		}
		dataSize += sizeof(hnswlib::labeltype);
	}
	dataSize *= count;
	...
	auto expectedRegex = fmt::format(
		...
		count + (emptyVectors.empty() ? 0 : 1), dataSize, vectorIndex);
...
void FloatVector::TestHnswIndex() try {
	...
	constexpr static auto kFieldNameHnsw = "hnsw";
	...
	checkIndexMemstat(rt, kNsName, kFieldNameHnsw, kDimension, emptyVectors, vectorsCount, kM);
}
CATCH_AND_ASSERT
...
void FloatVector::TestVecBruteforceIndex() try {
	...
	constexpr static auto kFieldNameVec = "vec";
#if defined(RX_WITH_STDLIB_DEBUG)
	constexpr static size_t kDimension = isArray ? 4 : 16;
	constexpr static size_t kMaxElements = 300;
#elif defined(REINDEX_WITH_TSAN) || defined(REINDEX_WITH_ASAN)
	...
	checkIndexMemstat(rt, kNsName, kFieldNameVec, kDimension, emptyVectors, vectorsCount, 0);
}
CATCH_AND_ASSERT
...
void FloatVector::TestIvfIndex() try {
	...
	for (size_t iter = 0; iter < 2; ++iter) {
		...
		checkIndexMemstat(rt, kNsName, kFieldNameIvf, kDimension, emptyVectors, vectorsCount, std::nullopt);
		if (iter == 0) {
			rebuildCentroids();
		}
	}
...
TEST_F(FloatVector, DslQuery) try {
	...
	struct {
		...
		{Query("ns"sv).WhereKNN("hnsw", vec.View(), reindexer::HnswSearchParams{}.K(6'101).Ef(100'000)).SelectAllFields(),
		...
		{Query("ns"sv).WhereKNN("bf", vec.View(), reindexer::BruteForceSearchParams{}.K(8'317).Radius(1.0)).Select("vectors()"),
		...
		{Query("ns"sv).WhereKNN("ivf", vec.View(), reindexer::IvfSearchParams{}.K(5'125).NProbe(5)).Select("ivf").Select("vectors()"),
	...
	for (const auto& [query, expectedDsl] : testData) {
		const auto generatedDsl = reindexer::impl::Impl{query}->GetJSON();
...
TEST_F(FloatVector, SqlQuery) try {
	...
	struct {
		...
		{Query("ns"sv).WhereKNN("hnsw", vec.View(), reindexer::HnswSearchParams{}.K(4'291).Ef(100'000)).SelectAllFields(),
		 "SELECT *, vectors() FROM ns WHERE KNN(hnsw, [" + vecStr + "], k=4291, ef=100000)"},
		{Query("ns"sv).WhereKNN("bf", vec.View(), reindexer::BruteForceSearchParams{}.K(8'184).Radius(1.2f)).Select("vectors()"),
		 "SELECT vectors() FROM ns WHERE KNN(bf, [" + vecStr + "], k=8184, radius=1.2)"},
		{Query("ns"sv).WhereKNN("ivf", vec.View(), reindexer::IvfSearchParams{}.K(823).NProbe(5)).Select("hnsw").Select("vectors()"),
...
struct [[nodiscard]] QueryIndexParams<IndexType::IndexIvf> {
	QueryIndexParams(size_t maxElements, size_t dimension, reindexer::VectorMetric metric) {
		...
		std::ignore = opts.SetDimension(dimension).SetNCentroids(maxElements / 50).SetMetric(metric);
	}
	std::string name = "ivf";
...
struct [[nodiscard]] QueryIndexParams<IndexType::IndexHnsw> {
	QueryIndexParams(size_t maxElements, size_t dimension, reindexer::VectorMetric metric) {
		...
		std::ignore = opts.SetDimension(dimension).SetMetric(metric).SetEfConstruction(5).SetStartSize(maxElements / 2).SetM(8);
	}
...
struct [[nodiscard]] QueryIndexParams<IndexType::IndexVectorBruteforce> {
	QueryIndexParams(size_t maxElements, size_t dimension, reindexer::VectorMetric metric) {
		...
		std::ignore = opts.SetDimension(dimension).SetMetric(metric).SetStartSize(maxElements);
	}
...
void FloatVector::TestQueries() try {
	...
	constexpr static auto kFieldNameBool = "bool";
#if defined(RX_WITH_STDLIB_DEBUG)
	constexpr static size_t kDimension = isArray ? 4 : 16;
	constexpr static size_t kMaxElements = 300;
#elif defined(REINDEX_WITH_TSAN) || defined(REINDEX_WITH_ASAN)
...
void checkFieldValue<true>(gason::JsonNode& parsedJson, const reindexer::Item& item, const std::string& fieldPath,
	...
	if (expectedData.empty()) {
		...
		const auto jsonField = reindexer_tests_tools::findJsonField(parsedJson, fieldPath);
		if (jsonField.isNull() || (jsonField.isArray() && (gason::begin(jsonField) == gason::end(jsonField)))) {
			if (!expectedData.empty()) {
				ASSERT_TRUE(expectedData.size() == 1 && expectedData[0].IsEmpty());
			}
		} else if (jsonField.isArray() && gason::begin(jsonField) != gason::end(jsonField) && gason::begin(jsonField)->isNumber()) {
...
void FloatVector::TestUpdateIndex(reindexer::VectorMetric metric) try {
	...
	auto testQueryNoIndex = [&] {
		...
		result = rt.Select(reindexer::Query{kNsName}
							   ...
							   .Where(kFieldNameFloatVector, CondLt, reindexer_tests_tools::randBin<float>(-1'000'000, 1'000'000)));
	};
