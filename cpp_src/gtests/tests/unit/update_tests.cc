...
TEST_F(CompositeUpdate, CompositeAndArray) {
	{
		auto q = Query(default_namespace).Update().Set(kFieldV1, 10).Set(kFieldArray, {10, 11}).Where(kFieldId, CondEq, 1);
	...
	{
		auto q = Query(default_namespace).Update().Set(kFieldArray, {11, 11}).Set(kFieldV1, 11).Where(kFieldId, CondEq, 1);
		ExecuteAndCheckResult(q, R"({"id":1,"array":[11,11],"v1":11,"v2":200,"v3":1000,"v4":"str"})");
	}
	{
		auto q =
			Query(default_namespace).Update().Set(kFieldV1, 12).Set(kFieldArray, {12, 12}).Set(kFieldV4, "a").Where(kFieldId, CondEq, 1);
