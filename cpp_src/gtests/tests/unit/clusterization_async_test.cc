...
TEST_F(ClusterizationAsyncApi, AsyncReplicationForClusterNamespaces) {
	...
	loop.spawn([&loop, &defaults]() noexcept {
		...
		for (size_t i = 0; i < kClusterSize - 1; ++i) {
			...
			cluster.AddAsyncNode(i, asyncNodes[i].dsn, cluster::AsyncReplicationMode::Default);
		}
		...
		cluster.AddAsyncNode(kLastNode, asyncNodes[kLastNode].dsn, cluster::AsyncReplicationMode::Default);
...
TEST_F(ClusterizationAsyncApi, AsyncReplicationBetweenClustersLeaderMode) {
	...
	loop.spawn([&loop, &defaults]() noexcept {
		...
		for (unsigned i = 0; i < kClusterSize; ++i) {
			...
			for (unsigned j = 0; j < kClusterSize; ++j) {
				cluster1.AddAsyncNode(j, cluster2.GetNode(i)->kRPCDsn, cluster::AsyncReplicationMode::FromClusterLeader, {{kNs2}});
			}
		}
