...
TEST_F(ReindexerApi, ConcurrentRenaming) {
	...
	auto selectionTh2 = [&](std::string_view ns, std::string_view otherNs) {
		while (!done) {
			executeAndValidate(ns, otherNs, Query(ns).InnerJoin(Query(ns), "id", CondEq, "id").Sort("hash()", false).Limit(10), 10);
			std::this_thread::sleep_for(std::chrono::milliseconds(5));
		}
	};
	auto renameTh = [&, this]() {
...
TEST_F(ReindexerApi, SortByMultipleColumns) {
	...
	const Query query{Query(default_namespace, offset, limit).Sort("column1", true).Sort("column2", false).Sort("column3", false)};
...
TEST_F(ReindexerApi, SortByUnorderedIndexWithJoins) {
	...
	joinQuery.InnerJoin(std::move(querySecondNamespace), "fk", CondEq, "pk");
...
TEST_F(ReindexerApi, DistinctQueriesEncodingTest) {
	...
	QueryImpl q4 = QueryImpl::CreateEmpty();
	EXPECT_NO_THROW(q4 = QueryImpl::FromSQL(sql2));
...
TEST_F(ReindexerApi, UpdateWithBoolParserTest) {
	...
	const auto query = QueryImpl::FromSQL(sql);
...
TEST_F(ReindexerApi, SchemaSuggestions) {
	...
	struct {
		...
		{"select * from test_namespace where (select Field from second_ns where ",
		 {"id", "ST_DWithin", "Field", "not", "equal_position", "KNN", "flat_array_len"}},
...
TEST_F(ReindexerApi, SelectFilterWithAggregationConstraints) {
	using namespace std::string_view_literals;
	{
		QueryImpl q = QueryImpl::CreateEmpty();
		const auto sql = "select id, distinct(year) from test_namespace"sv;
		EXPECT_NO_THROW(q = QueryImpl::FromSQL(sql));

		QueryImpl qJSON = QueryImpl::CreateEmpty();
		EXPECT_NO_THROW(qJSON = QueryImpl::FromJSON(q.GetJSON()));
	}
...

	{
		...
		auto q = Query::FromSQL(sql);
		EXPECT_FALSE(Impl{q}.State().ok());
	}
	...
	{
		auto q = QueryImpl(default_namespace);
		q.Select("id");
		q.Select("name");
	...
	EXPECT_THROW(std::ignore = QueryImpl::FromSQL("select max(id), * from test_namespace"), Error);
	EXPECT_THROW(std::ignore = QueryImpl::FromSQL("select *, max(id) from test_namespace"), Error);
	EXPECT_NO_THROW(std::ignore = QueryImpl::FromSQL("select *, count(*) from test_namespace"));
	EXPECT_NO_THROW(std::ignore = QueryImpl::FromSQL("select count(*), * from test_namespace"));
}
