...
void ApiTvComposite::RangeTreeIntSortByTreeInt(State& state) {
	const auto q = [&] {
		const auto idRange = id_seq_->GetRandomIdRange(id_seq_->Count() * 0.02);
		return Query(nsdef_.name).Where("id", CondRange, {idRange.first, idRange.second}).Sort("year", false).Limit(20);
	};
	benchQuery(q, state);
}
...
void ApiTvComposite::RangeTreeStrSortByHashInt(benchmark::State& state) {
	const auto q = [&] {
		const auto idRange = id_seq_->GetRandomIdRange(id_seq_->Count() * 0.02);
		return Query(nsdef_.name)
			.Where("id", CondRange, {std::to_string(idRange.first), std::to_string(idRange.second)})
			.Sort("age", false)
			.Limit(20);
	};
	benchQuery(q, state);
...
void ApiTvComposite::RangeTreeDoubleSortByTreeInt(benchmark::State& state) {
	const auto q = [&] {
		...
		return Query(nsdef_.name).Where("rate", CondRange, {leftRate, rightRate}).Sort("year", false).Limit(20);
	};
	benchQuery(q, state);
}
...
void ApiTvComposite::RangeTreeDoubleSortByHashInt(benchmark::State& state) {
	const auto q = [&] {
		const auto leftRate = random<double>(0.0, 4.99);
		const auto rightRate = random<double>(5.0, 10.0);
		return Query(nsdef_.name).Where("rate", CondRange, {leftRate, rightRate}).Sort("age", false).Limit(20);
	};
	benchQuery(q, state);
}
...
void ApiTvComposite::SortByHashInt(benchmark::State& state) {
	const auto q = Query(nsdef_.name).Sort("id", false).Limit(20);
	benchQuery(q, state);
}

void ApiTvComposite::ForcedSortByHashInt(benchmark::State& state) {
	const auto q = Query(nsdef_.name).Sort("id", false, {10, 20, 30, 40, 50}).Limit(20);
	benchQuery(q, state);
}

void ApiTvComposite::ForcedSortWithSecondCondition(benchmark::State& state) {
	const auto q = Query(nsdef_.name).Sort("id", false, {10, 20, 30, 40, 50}).Sort("location", false).Limit(20);
	benchQuery(q, state);
}
...
void ApiTvComposite::SortByHashStrCollateASCII(benchmark::State& state) {
	const auto q = Query(nsdef_.name).Sort("location", false).Limit(20);
	benchQuery(q, state);
}

void ApiTvComposite::SortByHashStrCollateUTF8(benchmark::State& state) {
	const auto q = Query(nsdef_.name).Sort("name", false).Limit(20);
	benchQuery(q, state);
}

void ApiTvComposite::SortByHashCompositeIntInt(benchmark::State& state) {
	const auto q = Query(nsdef_.name).Sort("id+start_time", false).Limit(20);
	benchQuery(q, state);
}

void ApiTvComposite::SortByHashCompositeIntStr(benchmark::State& state) {
	const auto q = Query(nsdef_.name).Sort("id+genre", false).Limit(20);
	benchQuery(q, state);
}

void ApiTvComposite::SortByTreeCompositeIntInt(benchmark::State& state) {
	const auto q = Query(nsdef_.name).Sort("id+year", false).Limit(20);
	benchQuery(q, state);
}

void ApiTvComposite::SortByTreeCompositeIntStrCollateUTF8(benchmark::State& state) {
	const auto q = Query(nsdef_.name).Sort("id+name", false).Limit(20);
	benchQuery(q, state);
}
