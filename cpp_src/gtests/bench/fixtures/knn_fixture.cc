...
#if !defined(REINDEX_WITH_TSAN) && !defined(RX_WITH_STDLIB_DEBUG) && !defined(REINDEX_WITH_ASAN)
static constexpr size_t kDimention = 256;
static constexpr size_t kNsSize = 100'000;
#else	// !defined(REINDEX_WITH_TSAN) && !defined(RX_WITH_STDLIB_DEBUG) && !defined(REINDEX_WITH_ASAN)
static constexpr size_t kDimention = 32;
static constexpr size_t kNsSize = 10'000;
#endif	// !defined(REINDEX_WITH_TSAN) && !defined(RX_WITH_STDLIB_DEBUG) && !defined(REINDEX_WITH_ASAN)
...
public:
	using Base::Base;
	~ItemsCounter() {
		state_.counters["Items/Op"] = double(itemsCounter_) / state_.iterations();
...
void KnnBench<indexType, metric>::RegisterAllCases() {
	...
	Register("Fill"s, &KnnBench<indexType, metric>::Fill, this)->Iterations(1);
