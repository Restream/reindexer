...
void KnnBench<indexType, metric>::FillFtIndex(State& state) {
	...
	for (auto _ : state) {	// NOLINT(*deadcode.DeadStores)
		...
		for (int i = id_seq_->Start(); i < count; ++i) {
			auto q = Query(nsdef_.name).Update().Set("ft", CreatePhrase()).Where("id", CondEq, i);
			auto err = tx.Modify(*Impl{std::move(q)});
			if (!err.ok()) {
				state.SkipWithError(err.what());
			}
		}
		...
		}
		{
			// build ft index
			QueryResults qr;
			auto err = db_->Select(Query(nsdef_.name).Where("ft", CondEq, RndWord1()), qr);
			if (!err.ok()) {
				state.SkipWithError(err.what());
			}
		}
...
void KnnBench<indexType, metric>::Knn(State& state) {
	...
	const auto q = [&] {
		reindexer_tests_tools::rndFloatVector(vect);
		return Query(nsdef_.name).WhereKNN("vec", ConstFloatVectorView{vect}, KnnSearchParams<indexType, metric, knnParams>{}());
	};
...
void KnnBench<indexType, metric>::KnnWithVectors(State& state) {
	...
	const auto q = [&] {
		...
		return Query(nsdef_.name)
			.WhereKNN("vec", ConstFloatVectorView{vect}, KnnSearchParams<indexType, metric, knnParams>{}())
			.SelectAllFields();
	};
	ItemsCounter<knnParams> itemsCounter{state};
	benchQuery(q, state, itemsCounter);
...
void KnnBench<indexType, metric>::KnnWithCondition(State& state) {
	...
	const auto q = [&] {
		reindexer_tests_tools::rndFloatVector(vect);
		return Query(nsdef_.name)
			.Where("tree", CondGt, rand() % (kMaxValueOrTreeIndex / 2))
			.WhereKNN("vec", ConstFloatVectorView{vect}, KnnSearchParams<indexType, metric, knnParams>{}());
	};
	ItemsCounter<knnParams> itemsCounter{state};
	benchQuery(q, state, itemsCounter);
...
void KnnBench<indexType, metric>::KnnWith2Conditions(State& state) {
	...
	const auto q = [&] {
		reindexer_tests_tools::rndFloatVector(vect);
		return Query(nsdef_.name)
			.Where("id", CondGt, rand() % halfOfItemsCount)
			.Where("tree", CondGt, rand() % (kMaxValueOrTreeIndex / 2))
			.WhereKNN("vec", ConstFloatVectorView{vect}, KnnSearchParams<indexType, metric, knnParams>{}());
	};
	ItemsCounter<knnParams> itemsCounter{state};
...
void KnnBench<indexType, metric>::AndHybridRrf(State& state) {
	...
	const auto q = [&] {
		...
		return Query(nsdef_.name)
			.WhereKNN("vec", ConstFloatVectorView{vect}, KnnSearchParams<indexType, metric, knnParams>{}())
			.Where("ft", CondEq, RndWord1() + ' ' + RndWord1())
			.Sort("RRF()", false);
	};
	ItemsCounter<knnParams> itemsCounter{state};
	benchQuery(q, state, itemsCounter);
