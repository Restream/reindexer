...
void UpdateItems::scalarIndex(State& state, unsigned int limit) {
	auto qGen = [&]() {
		return reindexer::Query(nsdef_.name).Update().Set(std::string(kFieldScalar), randString(kStringLen)).Limit(limit);
	};
...
void UpdateItems::scalarSparseIndex(State& state, unsigned int limit) {
	auto qGen = [&]() {
		return reindexer::Query(nsdef_.name).Update().Set(std::string(kFieldScalarSparse), randString(kStringLen)).Limit(limit);
	};
	benchUpdate(qGen, state);
}
...
void UpdateItems::scalar(State& state, unsigned int limit) {
	auto qGen = [&]() {
		return reindexer::Query(nsdef_.name).Update().Set(std::string(kFieldScalarNotindex), randString(kStringLen)).Limit(limit);
	};
	benchUpdate(qGen, state);
}
...
void UpdateItems::arrayIndex(State& state, unsigned int limit) {
	auto qGen = [&]() {
		...
		return reindexer::Query(nsdef_.name).Update().Set(std::string(kFieldArray), array).Limit(limit);
	};
	benchUpdate(qGen, state);
}
...
void UpdateItems::arraySparseIndex(State& state, unsigned int limit) {
	auto qGen = [&]() {
		auto array = RandStrLocal(rand() % kMaxArrayLen + 2, kStringLen);
		return reindexer::Query(nsdef_.name).Update().Set(std::string(kFieldArraySparse), array).Limit(limit);
	};
	benchUpdate(qGen, state);
}
...
void UpdateItems::array(State& state, unsigned int limit) {
	auto qGen = [&]() {
		auto array = RandStrLocal(rand() % kMaxArrayLen + 2, kStringLen);
		return reindexer::Query(nsdef_.name).Update().Set(std::string(kFieldArrayNotindex), array).Limit(limit);
	};
	benchUpdate(qGen, state);
}
...
void UpdateItems::array2Index(State& state, unsigned int limit) {
	auto qGen = [&]() {
		...
		return reindexer::Query(nsdef_.name).Update().Set(path, val).Limit(limit);
	};
	benchUpdate(qGen, state);
}
...
void UpdateItems::threeIndexField(State& state, unsigned int limit) {
	auto qGen = [&]() {
		...
		return reindexer::Query(nsdef_.name)
			.Update()
			.Set(std::string(kFieldScalar), randString(kStringLen))
			.Set(std::string(kFieldArray), array)
			.Set(path, val)
			.Limit(limit);
	};
	benchUpdate(qGen, state);
...
void UpdateItems::threeIndexSparseField(State& state, unsigned int limit) {
	auto qGen = [&]() {
		...
		return reindexer::Query(nsdef_.name)
			.Update()
			.Set(std::string(kFieldScalarSparse), randString(kStringLen))
			.Set(std::string(kFieldArraySparse), array)
			.Set(path, val)
			.Limit(limit);
	};
	benchUpdate(qGen, state);
...
void UpdateItems::threeField(State& state, unsigned int limit) {
	auto qGen = [&]() {
		...
		return reindexer::Query(nsdef_.name)
			.Update()
			.Set(std::string(kFieldScalar), randString(kStringLen))
			.Set(std::string(kFieldArrayNotindex), array)
			.Set(path, val)
			.Limit(limit);
	};
	benchUpdate(qGen, state);
...
void UpdateItems::nineField(State& state, unsigned int limit) {
	auto qGen = [&]() {
		...
		return reindexer::Query(nsdef_.name)
			.Update()
			.Set(std::string(kFieldScalar), randString(kStringLen))
			.Set(std::string(kFieldArray), array3)
			.Set(path3, val3)
			.Set(std::string(kFieldScalarSparse), randString(kStringLen))
			.Set(std::string(kFieldArraySparse), array2)
			.Set(path2, val2)
			.Set(std::string(kFieldScalarNotindex), randString(kStringLen))
			.Set(std::string(kFieldArrayNotindex), array1)
			.Set(path1, val1)
			.Limit(limit);
	};
	benchUpdate(qGen, state);
