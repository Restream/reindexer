...
public:
	...
	std::pair<ShardIDsContainer, Variant> GetHostsIdsKeyPair(const impl::Query&) const;
...
private:
	bool getHostIdForQuery(const impl::Query&, int& currentId, Variant& shardKey) const;
	ShardingKeys keys_;
};
...
public:
	...
	std::pair<ShardIDsContainer, Variant> GetShardIdKeyPair(const impl::Query& q) const { return routingStrategy_.GetHostsIdsKeyPair(q); }
	...
	ConnectionsPtr GetShardsConnectionsWithId(const impl::Query& q, Error& status) RX_REQUIRES(!m_);
