...
public:
	...
	[[nodiscard]] bool OwnsThis(const Mtx1& mtx) const noexcept {
		return unique_lock<Mtx1>::owns_lock() && unique_lock<Mtx1>::mutex() == &mtx;
	}
	[[nodiscard]] bool OwnsThis(const Mtx2& mtx) const noexcept {
		return unique_lock<Mtx2>::owns_lock() && unique_lock<Mtx2>::mutex() == &mtx;
	}
