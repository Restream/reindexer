...
public:
	...
	[[nodiscard]] RX_ALWAYS_INLINE std::cv_status wait_for(unique_lock<Mutex>& lock, const std::chrono::duration<Rep, Period>& rel_time) {
		return Base::wait_for(lock, rel_time);
	}
	template <typename Mutex, typename Rep, typename Period, typename Predicate>
	[[nodiscard]] RX_ALWAYS_INLINE bool wait_for(unique_lock<Mutex>& lock, const std::chrono::duration<Rep, Period>& rel_time,
												 Predicate pred) {
