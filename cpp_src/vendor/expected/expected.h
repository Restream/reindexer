...
#define expected_lite_VERSION \
	expected_STRINGIFY(expected_lite_MAJOR) "." expected_STRINGIFY(expected_lite_MINOR) "." expected_STRINGIFY(expected_lite_PATCH)
...
constexpr auto make_unexpected(std::in_place_t inplace, Args&&... args) -> unexpected_type<typename std::decay<E>::type> {
...
#define nsel_deprecated(msg) /*[[deprecated]]*/
	...
	public:
		...
		void construct_value() { new (&m_value) value_type(); }
		...
		constexpr const value_type&& value() const&& { return std::move(m_value); }

		nsel_constexpr14 value_type&& value() && { return std::move(m_value); }

		const value_type* value_ptr() const { return &m_value; }

		value_type* value_ptr() { return &m_value; }

		const error_type& error() const& { return m_error; }

		error_type& error() & { return m_error; }

		constexpr const error_type&& error() const&& { return std::move(m_error); }
	...
	public:
		...
		void construct_value(const value_type& e) { new (&m_value) value_type(e); }
		...
		void construct_error(const error_type& e) { new (&m_error) error_type(e); }
	...
	public:
		...
		storage_t(const storage_t& other) : storage_t_impl<T, E>(other.has_value()) {
			if (this->has_value()) {
	...
	public:
		...
		storage_t(const storage_t& other) : storage_t_impl<void, E>(other.has_value()) {
			if (this->has_value())
				;
			else {
				this->construct_error(other.error());
	...
	nsel_constexpr auto invoke_member_function_impl(FnT ClassT::*memfnptr, ObjectT&& obj, Args&&... args) noexcept(noexcept(
		(std::forward<ObjectT>(obj).*memfnptr)(std::forward<Args>(args)...))) -> decltype((std::forward<ObjectT>(obj).*
	...
	template <typename FnT, typename ClassT, typename ObjectT,
			  typename... Args nsel_REQUIRES_T(
				  std::is_function<FnT>::value&& is_reference_wrapper<typename std20::remove_cvref<ObjectT>::type>::value)>
	nsel_constexpr auto invoke_member_function_impl(FnT ClassT::*memfnptr, ObjectT&& obj, Args&&... args) noexcept(
		noexcept((obj.get().*memfnptr)(std::forward<Args>(args)...))) -> decltype((obj.get().*memfnptr)(std::forward<Args>(args)...)) {
	...
	nsel_constexpr auto invoke_member_object_impl(MemberT ClassT::*memobjptr,
												  ObjectT&& obj) noexcept(noexcept(obj.get().*memobjptr)) -> decltype(obj.get().*
																													  memobjptr) {
	...
	template <typename F, typename... Args nsel_REQUIRES_T(std::is_member_function_pointer<typename std20::remove_cvref<F>::type>::value)>
	nsel_constexpr auto invoke(F&& f, Args&&... args) noexcept(noexcept(invoke_member_function_impl(std::forward<F>(f),
	...
	template <typename F, typename... Args nsel_REQUIRES_T(std::is_member_object_pointer<typename std20::remove_cvref<F>::type>::value)>
	nsel_constexpr auto invoke(F&& f, Args&&... args) noexcept(noexcept(invoke_member_object_impl(std::forward<F>(f),
																								  std::forward<Args>(args)...)))
	...
	struct valid_expected_value_type
		: std::integral_constant<bool, std::is_destructible<T>::value && !std::is_reference<T>::value && !std::is_array<T>::value> {};
	...
	public:
		...
		constexpr unexpected_type(const unexpected_type&) = default;
		...
		constexpr explicit unexpected_type(nonstd_lite_in_place_t(E), Args&&... args) : m_error(std::forward<Args>(args)...) {}

		template <typename U, typename... Args nsel_REQUIRES_T(std::is_constructible<E, std::initializer_list<U>, Args&&...>::value)>
		...
		template <typename E2 nsel_REQUIRES_T(
			...
			!std::is_constructible<E, unexpected_type<E2> >::value && !std::is_constructible<E, const unexpected_type<E2>&>::value &&
			!std::is_constructible<E, const unexpected_type<E2> >::value && !std::is_convertible<unexpected_type<E2>&, E>::value &&
			!std::is_convertible<unexpected_type<E2>, E>::value && !std::is_convertible<const unexpected_type<E2>&, E>::value &&
			!std::is_convertible<const unexpected_type<E2>, E>::value && !std::is_convertible<E2 const&, E>::value /*=> explicit */
			)>
		constexpr explicit unexpected_type(const unexpected_type<E2>& error) : m_error(E{error.error()}) {}
		...
		constexpr /*non-explicit*/ unexpected_type(const unexpected_type<E2>& error) : m_error(error.error()) {}
		...
		template <typename E2 nsel_REQUIRES_T(
			...
			!std::is_constructible<E, const unexpected_type<E2> >::value && !std::is_convertible<unexpected_type<E2>&, E>::value &&
			!std::is_convertible<unexpected_type<E2>, E>::value && !std::is_convertible<const unexpected_type<E2>&, E>::value &&
			!std::is_convertible<const unexpected_type<E2>, E>::value && std::is_convertible<E2 const&, E>::value /*=> non-explicit */
			)>
		constexpr /*non-explicit*/ unexpected_type(unexpected_type<E2>&& error) : m_error(std::move(error.error())) {}

		// x.x.5.2.2 Assignment

		nsel_constexpr14 unexpected_type& operator=(const unexpected_type&) = default;
		nsel_constexpr14 unexpected_type& operator=(unexpected_type&&) = default;

		template <typename E2 = E>
		nsel_constexpr14 unexpected_type& operator=(const unexpected_type<E2>& other) {
		...
		nsel_REQUIRES_R(void, std17::is_swappable<U>::value) swap(unexpected_type& other) noexcept(std17::is_nothrow_swappable<U>::value) {
			using std::swap;
	...
	public:
		...
		explicit unexpected_type(const std::exception_ptr& error) : m_error(error) {}
		...
		const std::exception_ptr& value() const { return m_error; }
	...
	constexpr bool operator==(const unexpected_type<E1>& x, const unexpected_type<E2>& y) {
	...
	inline constexpr bool operator<(const unexpected_type<std::exception_ptr>& /*x*/, const unexpected_type<std::exception_ptr>& /*y*/) {
		return false;
	}
	...
	template <typename E, typename... Args, typename = std::enable_if<std::is_constructible<E, Args...>::value> >
	nsel_constexpr14 auto make_unexpected(nonstd_lite_in_place_t(E), Args&&... args) -> unexpected_type<typename std::decay<E>::type> {
	...
	public:
		...
		virtual const char* what() const noexcept override { return "bad_expected_access"; }
	...
	namespace detail {
	inline bool text(const char* /*text*/) { return true; }
	}  // namespace detail

	template <typename Error>
	struct error_traits {
		static void rethrow(const Error& /*e*/) {
#if nsel_CONFIG_NO_EXCEPTIONS_SEH
	...
	struct error_traits<std::exception_ptr> {
		static void rethrow(const std::exception_ptr& /*e*/) {
#if nsel_CONFIG_NO_EXCEPTIONS_SEH
			RaiseException(EXCEPTION_ACCESS_VIOLATION, EXCEPTION_NONCONTINUABLE, 0, NULL);
#else
	...
	struct error_traits<std::error_code> {
		static void rethrow(const std::error_code& /*e*/) {
#if nsel_CONFIG_NO_EXCEPTIONS_SEH
			RaiseException(EXCEPTION_ACCESS_VIOLATION, EXCEPTION_NONCONTINUABLE, 0, NULL);
#else
	...
	template <typename Error>
	struct error_traits {
		static void rethrow(const Error& e) { throw bad_expected_access<Error>{e}; }
	};

	template <>
	struct error_traits<std::exception_ptr> {
		static void rethrow(const std::exception_ptr& e) { std::rethrow_exception(e); }
	};

	template <>
	struct error_traits<std::error_code> {
		static void rethrow(const std::error_code& e) { throw std::system_error(e); }
	};
	...
	// "the type of the unexpected value. The type must not be an array type, a non-object type, a specialization of std::unexpected, or a
	// cv-qualified type."
	template <typename T>
	struct valid_unexpected_type : std::integral_constant<bool, std::is_same<T, typename std20::remove_cvref<T>::type>::value &&
	...
	public:
		...
		nsel_constexpr14 expected(const expected&) = default;
		...
		template <typename U,
				  typename G nsel_REQUIRES_T(
					  ...
					  (!std::is_convertible<const U&, T>::value || !std::is_convertible<const G&, E>::value) /*=> explicit */
					  )>
		nsel_constexpr14 explicit expected(const expected<U, G>& other) : contained(other.has_value()) {
			if (has_value()) {
		...
		template <typename U,
				  typename G nsel_REQUIRES_T(
					  ...
					  !std::is_convertible<expected<U, G>&, T>::value && !std::is_convertible<expected<U, G>&&, T>::value &&
					  !std::is_convertible<const expected<U, G>&, T>::value && !std::is_convertible<const expected<U, G>&&, T>::value &&
					  !(!std::is_convertible<const U&, T>::value || !std::is_convertible<const G&, E>::value) /*=> non-explicit */
					  )>
		nsel_constexpr14 /*non-explicit*/ expected(const expected<U, G>& other) : contained(other.has_value()) {
			if (has_value()) {
		...
		nsel_constexpr14 expected(const value_type& value) : contained(true) {
			contained.construct_value(value);
		}

		template <typename U = T nsel_REQUIRES_T(std::is_constructible<T, U&&>::value &&
		...
		template <typename G = E nsel_REQUIRES_T(std::is_constructible<E, const G&>::value &&
												 !std::is_convertible<const G&, E>::value /*=> explicit */
												 )>
		nsel_constexpr14 explicit expected(const nonstd::unexpected_type<G>& error) : contained(false) {
			contained.construct_error(E{error.error()});
		}

		template <typename G = E nsel_REQUIRES_T(
					  std::is_constructible<E, const G&>::value&& std::is_convertible<const G&, E>::value /*=> non-explicit */
					  )>
		nsel_constexpr14 /*non-explicit*/ expected(const nonstd::unexpected_type<G>& error) : contained(false) {
			contained.construct_error(error.error());
		}

		template <typename G = E nsel_REQUIRES_T(std::is_constructible<E, G&&>::value && !std::is_convertible<G&&, E>::value /*=> explicit
																															  */
												 )>
		...
		template <typename G = E nsel_REQUIRES_T(std::is_constructible<E, G&&>::value&& std::is_convertible<G&&, E>::value /*=> non-explicit
																															*/
												 )>
		...
		nsel_constexpr14 explicit expected(nonstd_lite_in_place_t(T), std::initializer_list<U> il, Args&&... args) : contained(true) {
		...
		// is_trivially_destructible_v<E> is false and !bool(*this), calls unexpect.~unexpected<E>(). Remarks: If either T is cv void or
		// is_trivially_destructible_v<T> is true, and is_trivially_destructible_v<E> is true, then this destructor shall be a trivial
		// destructor.
		...
		expected& operator=(const nonstd::unexpected_type<G>& error) {
		...
		template <typename U,
				  typename... Args nsel_REQUIRES_T(std::is_nothrow_constructible<T, std::initializer_list<U>&, Args&&...>::value)>
		...
		nsel_REQUIRES_R(void, std17::is_swappable<U>::value&& std17::is_swappable<G>::value &&
			...
			swap(expected& other) noexcept(std::is_nothrow_move_constructible<T>::value && std17::is_nothrow_swappable<T&>::value &&
		...
		constexpr const value_type* operator->() const { return assert(has_value()), contained.value_ptr(); }

		value_type* operator->() { return assert(has_value()), contained.value_ptr(); }

		constexpr const value_type& operator*() const& { return assert(has_value()), contained.value(); }

		value_type& operator*() & { return assert(has_value()), contained.value(); }

#if !nsel_COMPILER_GNUC_VERSION || nsel_COMPILER_GNUC_VERSION >= 490

		constexpr const value_type&& operator*() const&& { return std::move((assert(has_value()), contained.value())); }
		...
		nsel_DISABLE_MSVC_WARNINGS(4702)  // warning C4702: unreachable code, see issue 65.
			...
			return has_value() ? (contained.value()) : (error_traits<error_type>::rethrow(contained.error()), contained.value());
		}
		...
		nsel_RESTORE_MSVC_WARNINGS()
			...
			constexpr const value_type&& value() const&& {
			return std::move(has_value() ? (contained.value()) : (error_traits<error_type>::rethrow(contained.error()), contained.value()));
		}
		...
		constexpr const error_type& error() const& { return assert(!has_value()), contained.error(); }

		error_type& error() & { return assert(!has_value()), contained.error(); }

#if !nsel_COMPILER_GNUC_VERSION || nsel_COMPILER_GNUC_VERSION >= 490

		constexpr const error_type&& error() const&& { return std::move((assert(!has_value()), contained.error())); }
		...
		template <typename F nsel_REQUIRES_T(detail::is_expected<detail::invoke_result_nocvref_t<F, value_type&> >::value&&
												 std::is_same<typename detail::invoke_result_nocvref_t<F, value_type&>::error_type,
		...
		template <typename F nsel_REQUIRES_T(detail::is_expected<detail::invoke_result_nocvref_t<F, const value_type&> >::value&& std::
												 is_same<typename detail::invoke_result_nocvref_t<F, const value_type&>::error_type,
		...
		nsel_constexpr detail::invoke_result_nocvref_t<F, const value_type&> and_then(F&& f) const& {
			return has_value() ? detail::invoke_result_nocvref_t<F, const value_type&>(detail::invoke(std::forward<F>(f), value()))
		...
		template <typename F nsel_REQUIRES_T(detail::is_expected<detail::invoke_result_nocvref_t<F, value_type&&> >::value&&
												 std::is_same<typename detail::invoke_result_nocvref_t<F, value_type&&>::error_type,
															  error_type>::value&& std::is_constructible<error_type, error_type&&>::value)>
		nsel_constexpr14 detail::invoke_result_nocvref_t<F, value_type&&> and_then(F&& f) && {
			return has_value() ? detail::invoke_result_nocvref_t<F, value_type&&>(detail::invoke(std::forward<F>(f), std::move(value())))
		...
		template <typename F nsel_REQUIRES_T(detail::is_expected<detail::invoke_result_nocvref_t<F, error_type&> >::value&&
												 std::is_same<typename detail::invoke_result_nocvref_t<F, error_type&>::value_type,
		...
		template <typename F nsel_REQUIRES_T(detail::is_expected<detail::invoke_result_nocvref_t<F, const error_type&> >::value&& std::
												 is_same<typename detail::invoke_result_nocvref_t<F, const error_type&>::value_type,
		...
		nsel_constexpr14 expected<void, error_type> transform(F&& f) & {
			return has_value() ? (detail::invoke(std::forward<F>(f), **this), expected<void, error_type>()) : make_unexpected(error());
		}
		...
		template <typename F nsel_REQUIRES_T(detail::valid_unexpected_type<detail::transform_invoke_result_t<F, error_type&> >::value&&
		...
		template <typename F nsel_REQUIRES_T(detail::valid_unexpected_type<detail::transform_invoke_result_t<F, const error_type&> >::
		...
		nsel_constexpr expected<value_type, detail::transform_invoke_result_t<F, const error_type&> > transform_error(F&& f) const& {
			return has_value() ? expected<value_type, detail::transform_invoke_result_t<F, const error_type&> >(in_place, **this)
							   : make_unexpected(detail::invoke(std::forward<F>(f), error()));
		}
		...
		template <typename F nsel_REQUIRES_T(detail::valid_unexpected_type<detail::transform_invoke_result_t<F, error_type&&> >::value&&
												 std::is_constructible<value_type, value_type&&>::value)>
		nsel_constexpr14 expected<value_type, detail::transform_invoke_result_t<F, error_type&&> > transform_error(F&& f) && {
			return has_value() ? expected<value_type, detail::transform_invoke_result_t<F, error_type&&> >(in_place, std::move(**this))
	...
	public:
		...
		nsel_constexpr14 expected(const expected& other) = default;
		nsel_constexpr14 expected(expected&& other) = default;

		constexpr explicit expected(nonstd_lite_in_place_t(void)) : contained(true) {}

		template <typename G = E nsel_REQUIRES_T(!std::is_convertible<const G&, E>::value /*=> explicit */
												 )>
		nsel_constexpr14 explicit expected(const nonstd::unexpected_type<G>& error) : contained(false) {
			contained.construct_error(E{error.error()});
		}

		template <typename G = E nsel_REQUIRES_T(std::is_convertible<const G&, E>::value /*=> non-explicit */
												 )>
		nsel_constexpr14 /*non-explicit*/ expected(const nonstd::unexpected_type<G>& error) : contained(false) {
			contained.construct_error(error.error());
		}
		...
		nsel_REQUIRES_R(void, std17::is_swappable<G>::value&& std::is_move_constructible<G>::value)
			swap(expected& other) noexcept(std::is_nothrow_move_constructible<E>::value && std17::is_nothrow_swappable<E&>::value) {
			using std::swap;
...
#if nsel_P2505R >= 3
		// Monadic operations (P2505)
		template <typename F nsel_REQUIRES_T(detail::is_expected<detail::invoke_result_nocvref_t<F> >::value&&
		...
		template <
			typename F nsel_REQUIRES_T(detail::is_expected<detail::invoke_result_nocvref_t<F, const error_type&> >::value&&
...
#if !nsel_COMPILER_GNUC_VERSION || nsel_COMPILER_GNUC_VERSION >= 490
		template <typename F nsel_REQUIRES_T(detail::is_expected<detail::invoke_result_nocvref_t<F, error_type&&> >::value&& std::is_void<
		...
		nsel_constexpr14 expected<detail::transform_invoke_result_t<F>, error_type> transform(F&& f) & {
			return has_value() ? expected<detail::transform_invoke_result_t<F>, error_type>(detail::invoke(std::forward<F>(f)))
							   : make_unexpected(error());
		}
		...
		nsel_constexpr14 expected<void, error_type> transform(F&& f) & {
			return has_value() ? (detail::invoke(std::forward<F>(f)), expected<void, error_type>()) : make_unexpected(error());
		}
		...
		template <
			typename F nsel_REQUIRES_T(detail::valid_unexpected_type<detail::transform_invoke_result_t<F, const error_type&> >::value)>
	...
	constexpr bool operator==(const expected<T1, E1>& x, T2 const& v) {
		return bool(x) ? *x == v : false;
	}

	template <typename T1, typename E1, typename T2 nsel_REQUIRES_T(!std::is_void<T1>::value)>
	constexpr bool operator==(T2 const& v, const expected<T1, E1>& x) {
	...
	constexpr bool operator==(const expected<T1, E1>& x, const unexpected_type<E2>& u) {
		return (!x) ? x.get_unexpected() == u : false;
	}

	template <typename T1, typename E1, typename E2>
	constexpr bool operator==(const unexpected_type<E2>& u, const expected<T1, E1>& x) {
		return (x == u);
	}
	...
	template <typename T, typename E>
	constexpr bool operator<(const expected<T, E>& x, const unexpected_type<E>& u) {
		return (!x) ? (x.get_unexpected() < u) : false;
	}

	template <typename T, typename E>
	constexpr bool operator<(const unexpected_type<E>& u, const expected<T, E>& x) {
...
struct hash<nonstd::expected<T, E> > {
	...
	constexpr result_type operator()(const argument_type& arg) const { return arg ? std::hash<T>{}(*arg) : result_type{}; }
};
