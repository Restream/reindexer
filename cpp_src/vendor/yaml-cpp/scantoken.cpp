...
void Scanner::ScanDirective() {
	...
	// read name
	while (INPUT && !Exp::BlankOrBreak().Matches(INPUT)) token.value += INPUT.get();
	...
	while (true) {
		...
		if (!INPUT || Exp::Break().Matches(INPUT) || Exp::Comment().Matches(INPUT)) break;
...
void Scanner::ScanFlowEnd() {
	...
	if (m_flows.top() != flowType) throw ParserException(mark, ErrorMsg::FLOW_END);
	m_flows.pop();
...
void Scanner::ScanBlockEntry() {
	...
	if (InFlowContext()) throw ParserException(INPUT.mark(), ErrorMsg::BLOCK_ENTRY);

	// can we put it here?
	if (!m_simpleKeyAllowed) throw ParserException(INPUT.mark(), ErrorMsg::BLOCK_ENTRY);
...
void Scanner::ScanKey() {
	...
	if (InBlockContext()) {
		if (!m_simpleKeyAllowed) throw ParserException(INPUT.mark(), ErrorMsg::MAP_KEY);
...
void Scanner::ScanValue() {
	...
	if (isSimpleKey) {
		...
		if (InBlockContext()) {
			if (!m_simpleKeyAllowed) throw ParserException(INPUT.mark(), ErrorMsg::MAP_VALUE);
...
void Scanner::ScanAnchorOrAlias() {
	...
	if (name.empty()) throw ParserException(INPUT.mark(), alias ? ErrorMsg::ALIAS_NOT_FOUND : ErrorMsg::ANCHOR_NOT_FOUND);
...
void Scanner::ScanBlockScalar() {
	...
	for (int i = 0; i < n; i++) {
		...
		if (ch == '+')
			...
			if (ch == '0') throw ParserException(INPUT.mark(), ErrorMsg::ZERO_INDENT_IN_BLOCK);
	...
	if (INPUT && !Exp::Break().Matches(INPUT)) throw ParserException(INPUT.mark(), ErrorMsg::CHAR_IN_BLOCK);
