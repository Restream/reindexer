...
bool IsValidPlainScalar(const std::string& str, FlowType::value flowType, bool allowOnlyAscii) {
	...
	static const RegEx& disallowed_flow = Exp::EndScalarInFlow() | (Exp::BlankOrBreak() + Exp::Comment()) | Exp::NotPrintable() |
										  Exp::Utf8_ByteOrderMark() | Exp::Break() | Exp::Tab() | Exp::Ampersand();
...
bool IsValidSingleQuotedScalar(const std::string& str, bool escapeNonAscii) {
	...
	return std::none_of(str.begin(), str.end(),
						[=](char ch) { return (escapeNonAscii && (0x80 <= static_cast<unsigned char>(ch))) || (ch == '\n'); });
}
...
bool IsValidLiteralScalar(const std::string& str, FlowType::value flowType, bool escapeNonAscii) {
	...
	// TODO: check for non-printable characters?
	return std::none_of(str.begin(), str.end(), [=](char ch) { return (escapeNonAscii && (0x80 <= static_cast<unsigned char>(ch))); });
}
...
void WriteDoubleQuoteEscapeSequence(ostream_wrapper& out, int codePoint, StringEscaping::value stringEscapingStyle) {
	...
	for (; digits > 0; --digits) out << hexDigits[(codePoint >> (4 * (digits - 1))) & 0xF];
}
...
bool WriteDoubleQuotedString(ostream_wrapper& out, const std::string& str, StringEscaping::value stringEscaping) {
	out << "\"";
	int codePoint;
	for (std::string::const_iterator i = str.begin(); GetNextCodePointAndAdvance(codePoint, i, str.end());) {
		switch (codePoint) {
