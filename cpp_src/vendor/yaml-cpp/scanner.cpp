...
namespace YAML {
Scanner::Scanner(std::istream& in, const ScannerOpts& opts)
	: INPUT(in),
	  m_opts(opts),
	  m_tokens{},
...
Token& Scanner::peek() {
	EnsureTokensInQueue();
	assertrx(!m_tokens.empty());  // should we be asserting here? I mean, we really
								  // just be checking
...
void Scanner::ScanNextToken() {
	...
	if ((InBlockContext() ? Exp::PlainScalar() : Exp::PlainScalarInFlow()).Matches(INPUT) ||
		(m_opts.disableAnchors && (INPUT.peek() == Keys::Alias || INPUT.peek() == Keys::Anchor))) {
