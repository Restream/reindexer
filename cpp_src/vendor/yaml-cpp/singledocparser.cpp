...
SingleDocParser::SingleDocParser(Scanner& scanner, const Directives& directives)
	: m_scanner(scanner), m_directives(directives), m_pCollectionStack(new CollectionStack), m_anchors{}, m_curAnchor(0) {}
...
void SingleDocParser::HandleDocument(EventHandler& eventHandler) {
	...
	while (!m_scanner.empty() && m_scanner.peek().type == Token::DOC_END) m_scanner.pop();
}
...
void SingleDocParser::HandleNode(EventHandler& eventHandler) {
	...
	if (!anchor_name.empty()) eventHandler.OnAnchor(mark, anchor_name);
...
void SingleDocParser::HandleBlockSequence(EventHandler& eventHandler) {
	...
	while (true) {
		...
		if (token.type != Token::BLOCK_ENTRY && token.type != Token::BLOCK_SEQ_END) throw ParserException(token.mark, ErrorMsg::END_OF_SEQ);
...
void SingleDocParser::HandleBlockMap(EventHandler& eventHandler) {
	...
	while (true) {
		if (m_scanner.empty()) throw ParserException(m_scanner.mark(), ErrorMsg::END_OF_MAP);
...
void SingleDocParser::HandleFlowMap(EventHandler& eventHandler) {
	...
	while (true) {
		if (m_scanner.empty()) throw ParserException(m_scanner.mark(), ErrorMsg::END_OF_MAP_FLOW);
