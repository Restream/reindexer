...
void node_data::compute_seq_size() const {
	while (m_seqSize < m_sequence.size() && m_sequence[m_seqSize]->is_defined()) m_seqSize++;
}
...
void node_data::compute_map_size() const {
	...
	while (it != m_undefinedPairs.end()) {
		auto jt = std::next(it);
		if (it->first->is_defined() && it->second->is_defined()) m_undefinedPairs.erase(it);
		it = jt;
	}
}
...
bool node_data::remove(node& key, const shared_memory_holder& /* pMemory */) {
	...
	for (auto it = m_undefinedPairs.begin(); it != m_undefinedPairs.end();) {
		auto jt = std::next(it);
		if (it->first->is(key)) m_undefinedPairs.erase(it);
		it = jt;
	}

	auto it =
		std::find_if(m_map.begin(), m_map.end(), [&](std::pair<YAML::detail::node*, YAML::detail::node*> j) { return (j.first->is(key)); });
...
void node_data::insert_map_pair(node& key, node& value) {
	...
	if (!key.is_defined() || !value.is_defined()) m_undefinedPairs.emplace_back(&key, &value);
}
