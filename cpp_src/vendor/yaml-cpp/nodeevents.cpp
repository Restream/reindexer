...
NodeEvents::NodeEvents(const Node& node) : m_pMemory(node.m_pMemory), m_root(node.m_pNode), m_refCount{} {
...
void NodeEvents::Setup(const detail::node& node) {
	...
	if (node.type() == NodeType::Sequence) {
		for (auto element : node) Setup(*element);
...
void NodeEvents::Emit(const detail::node& node, EventHandler& handler, AliasManager& am) const {
	...
	switch (node.type()) {
		...
		case NodeType::Sequence:
			...
			for (auto element : node) Emit(*element, handler, am);
