...
inline std::string to_string(const Error error) {
  ...
  case Error::SSLPeerCouldBeClosed_: assert(false); // For internal use only
  default: break;
  }
...
inline bool ClientImpl::is_socket_open() const {
  std::lock_guard<std::mutex> guard(socket_mutex_);
...
inline bool Client::is_socket_open() const { return cli_->is_socket_open(); }
