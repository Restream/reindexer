...
SPDLOG_INLINE void logger::err_handler_(const std::string &msg) {
    if (custom_err_handler_) {
        ...
        using reindexer::system_clock_w; // Using alternative clocks to avoid vDSO issues on some systems
        static std::mutex mutex;
        static system_clock_w::time_point last_report_time;
        ...
        auto tm_time = details::os::localtime(system_clock_w::to_time_t(now));
        char date_buf[64];
