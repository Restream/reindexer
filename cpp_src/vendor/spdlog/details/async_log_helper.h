...
class async_log_helper {
	...
	struct async_msg {
		...
		explicit async_msg(async_msg_type m_type) : level(level::info), thread_id(0), msg_type(m_type), msg_id(0) {}
...
inline void spdlog::details::async_log_helper::push_msg(details::async_log_helper::async_msg&& new_msg) {
	if (!_q.enqueue(std::move(new_msg)) &&
		(_overflow_policy != async_overflow_policy::discard_log_msg || new_msg.msg_type != async_msg_type::log)) {
...
inline void spdlog::details::async_log_helper::set_formatter(formatter_ptr msg_formatter) { _formatter = std::move(msg_formatter); }
